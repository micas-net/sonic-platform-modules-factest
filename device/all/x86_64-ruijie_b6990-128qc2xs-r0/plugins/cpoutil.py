...
def test():
    cpo_api = cpoutilbase()
    cpo_api.set_speed(3, 200000)
    for port_index in range(1,1+128):
        cpo_api.set_speed(port_index, 200000)
        print(port_index, cpo_api.get_speed(port_index))

def oe_speed_test(speed, oe):

    if oe == None:
        oe_list = list(range(1,8+1))
    else:
        oe_list = [int(oe)]
    ...
    for oe in oe_list:
        try:
            if not cpoutil._file_rw_lock(oe):
                return False
            bus_id = 24 + oe - 1
            # [read|write, bus, offset, value, sleep_time(m second)]
            cmd_list = [
                # **set LowPwrRequestSW BankBoradcastEnable
                ["read", bus_id, None, None, 500],
                ["write", bus_id, 0x1a, 0x90, 500],
                ["read", bus_id, None, None, 500],

                # ***switch page to 0x10
                ["write", bus_id, 0x7f, 0x10, 500],
                ["read", bus_id, None, None, 500],

                # ***apsel lane 1-4
                ["write", bus_id, 0x91, apcode_a, 500],
                ["read", bus_id, None, None, 500],
                ["write", bus_id, 0x92, apcode_a, 500],
                ["read", bus_id, None, None, 500],
                ["write", bus_id, 0x93, apcode_a, 500],
                ["read", bus_id, None, None, 500],
                ["write", bus_id, 0x94, apcode_a, 500],
                ["read", bus_id, None, None, 500],

                # ***apsel lane 5-8
                ["write", bus_id, 0x95, apcode_b, 500],
                ["read", bus_id, None, None, 500],
                ["write", bus_id, 0x96, apcode_b, 500],
                ["read", bus_id, None, None, 500],
                ["write", bus_id, 0x97, apcode_b, 500],
                ["read", bus_id, None, None, 500],
                ["write", bus_id, 0x98, apcode_b, 500],
                ["read", bus_id, None, None, 500],

                # ***Apply_DataPathInit(init)
                ["write", bus_id, 0x8f, 0xff, 1000],
                ["read", bus_id, None, None, 500],

                # ***Data path initialization 
                ["write", bus_id, 0x80, 0x00, 1000],
                ["read", bus_id, None, None, 500],

                # ***Tx output squelch permitted 
                ["write", bus_id, 0x83, 0x00, 1000],
                ["read", bus_id, None, None, 500],

                # ***set rlm a enter full power mode
                ["write", bus_id, 0x7f, 0xb2, 500],
                ["read", bus_id, None, None, 500],
                ["write", bus_id, 0x80, 0xff, 500],
                ["read", bus_id, None, None, 500],

                # ***set rlm b enter full power mode
                ["write", bus_id, 0x7f, 0xb6, 500],
                ["read", bus_id, None, None, 500],
                ["write", bus_id, 0x80, 0xff, 500],
                ["read", bus_id, None, None, 500],

                # ***set oe enable 
                ["write", bus_id, 0x1a, 0x80, 500],
                ["read", bus_id, None, None, 500],

                # # ***switch page to 0x11 for check ret
                # ["write", bus_id, 0x7f, 0x11, 500],
                # ["read", bus_id, None, None, 500],
            ]

            for cmd in cmd_list:
                if cmd[0] == "read":
                    cmd_shell = "i2cdump -f -y {} 0x50\n".format(cmd[1])
                    print("cmd: ", cmd_shell)
                    os.system(cmd_shell)
                    print("sleep {} seconds".format(cmd[4]/1000))
                    time.sleep(cmd[4]/1000)

                if cmd[0] == "write":
                    print("\n\n\n\n")
                    cmd_shell = "i2cset -f -y {} 0x50 {} {}\n".format(cmd[1], hex(cmd[2]), hex(cmd[3]))
                    print("cmd: ", cmd_shell)
                    os.system(cmd_shell)
                    print("sleep {} seconds".format(cmd[4]/1000))
                    time.sleep(cmd[4]/1000)
            ...
            cpoutil._file_rw_unlock(oe)

    return True

if __name__=="__main__":
    # Get the list of command line arguments with sys.argv
    args = sys.argv[1:] # Remove the first element (script name)
    
    if len(args) > 1:
        print("The input parameters are:", args)
        speed = int(args[0])
        oe = int(args[1])
        oe_speed_test(speed, oe)
    elif len(args) > 0:
        print("The input parameters are:", args)
        speed = int(args[0])
        oe_speed_test(speed, None)
    else:
