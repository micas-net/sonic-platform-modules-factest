...
DEV_MONITOR_PARAM = {
    ...
    "fans": [
        ...
        },
        {
            ...
            "device": [
                {"id": "fan3frue2", "name": "wb_24c02", "bus": 5, "loc": 0x53, "attr": "eeprom"},
            ],
        },
        {
            "name": "fan4",
            ...
            "device": [
                {"id": "fan4frue2", "name": "wb_24c02", "bus": 6, "loc": 0x53, "attr": "eeprom"},
            ],
        },
    ],
    "others": [
...
REBOOT_CAUSE_PARA = {
    "reboot_cause_list": [
        ...
        {
            "name": "chang_sonic_reboot",
            "monitor_point": {"gettype": "file_exist", "judge_file": "/tmp/notify_firstboot_to_platform", "okval": True},
            "record": [
                {"record_type": "file", "mode": "cover", "log": "First Boot of This Sonic version, ",
                    "path": "/etc/sonic/.reboot/.previous-reboot-cause.txt"},
                {"record_type": "file", "mode": "add", "log": "First Boot of This Sonic version, ",
                    "path": "/etc/sonic/.reboot/.history-reboot-cause.txt", "file_max_size": 1 * 1024 * 1024}
            ],
        },
        {
...
DRIVERLISTS = [
    ...
    # x86 embedded legacy smbus
    {"name": "wb_i2c_i801", "delay": 1},
    
    # gpio drv
    {"name": "wb_gpio_d1500", "delay": 0},

    # gpio dev, it relates to wb_gpio_d1500 drv
    ...
    # gpio i2c dev, it relates to gpio i2c drv
    ...
    # temp sensor lm75 drv
    {"name": "wb_lm75", "delay": 0},

    # sfp relatived optoe drv
    ...
    # power sysfs attr export relatived drv
    {"name": "wb_pmbus_core", "delay": 0},
    {"name": "wb_csu550", "delay": 0},

    # to do adapt
    #{"name": "rg_reset_mac", "delay": 0},
    #{"name": "rg_reset_mac_device", "delay": 0},
    ...
    {"name": "platform_common dfd_my_type=0x4040", "delay": 0},
    ...
    # no curr sensor, don't load. if load, it will load fail
...
INIT_COMMAND = [
    ...
    "i2cset -y -f 1 0x34 0x60 0x0",   # sfp_txdis1
    "i2cset -y -f 1 0x34 0x61 0x0",   # sfp_txdis2
    "i2cset -y -f 1 0x34 0x62 0x0",   # sfp_txdis3
    "i2cset -y -f 1 0x36 0x60 0x0",   # sfp_txdis4
    "i2cset -y -f 1 0x36 0x61 0x0",   # sfp_txdis5
    "i2cset -y -f 1 0x36 0x62 0x0",   # sfp_txdis6
    "i2cset -y -f 0 0x32 0x15 0x80",  # fan_speed_set
    "i2cset -y -f 2 0x33 0xad 0xad",  # sfp_led1_yellow
    "i2cset -y -f 2 0x35 0xad 0xad",  # sfp_led2_yellow
]
...
UPGRADE_SUMMARY = {
    ...
    "slot0": {
        ...
        "TEST": {
            "cpld": [
                {"chain": 1, "file": "/etc/.upgrade_test/b6510_48_cpld_test_0_1_2000_header.vme", "display_name": "CPLD"},
            ],
        },
    },
}
