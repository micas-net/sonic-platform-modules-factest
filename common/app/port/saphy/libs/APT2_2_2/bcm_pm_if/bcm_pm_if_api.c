...
bcm_plp_phy_static_config_t bcm_plp_aperta2_phy_static_config[PHYMOD_IF_CONFIG_MAX_PHYS] = {{0xff,NULL},}; 
 ...
 */
int
bcm_plp_aperta2_mutex_info_set(bcm_plp_access_t phy_info, bcm_plp_mutex_info_t *mutex_info)
{
    ...
    /* Create PHY object */
    if ((plp_aperta2_phy_ctrl.phy[phy_info.phy_addr] == NULL) || (plp_aperta2_phy_ctrl.phy[phy_info.phy_addr]->valid != 1)) { 
        rv = _bcm_plp_aperta2_if_phymod_phy_create(&plp_aperta2_phy_ctrl.phy[phy_info.phy_addr]);
        if (rv != 0) {
            return rv;
        }
    }

    _plp_aperta2_phyid_list[phy_info.phy_addr].phy_id = phy_info.phy_addr; 
    plp_aperta2_phy_ctrl.phy[phy_info.phy_addr]->mutex_info.mutex_give = mutex_info->mutex_give;
    plp_aperta2_phy_ctrl.phy[phy_info.phy_addr]->mutex_info.mutex_take = mutex_info->mutex_take;
 ...
 */
int
bcm_plp_aperta2_static_config_set(bcm_plp_access_t phy_info, void* bcm_static_config)
{
    ...
    for(glb_arr_idx = 0; glb_arr_idx < PHYMOD_IF_CONFIG_MAX_PHYS; glb_arr_idx++) {
        if(bcm_plp_aperta2_phy_static_config[glb_arr_idx].phy_id == 0xff) {
            if(glb_arr_idx == 0) {
                /* Initialize all entries from index 1 to 1023 */
                for (index = 1; index < PHYMOD_IF_CONFIG_MAX_PHYS; index ++) {
                    if(bcm_plp_aperta2_phy_static_config[index].bcm_static_config == NULL) {
                        bcm_plp_aperta2_phy_static_config[index].phy_id = 0xff;
                    }
                }    
            }
            if(bcm_static_config) {
                bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config = PHYMOD_IF_MALLOC(sizeof(plp_static_config_t));
                if(bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config == NULL) {
                    BCM_PM_RETURN_WITH_ERR(BCM_PM_IF_MEMORY, (_PHYMOD_MSG("Null parameter")));
                }
                PHYMOD_IF_MEMSET(bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config, 0, sizeof(plp_static_config_t));
            } 
            bcm_plp_aperta2_phy_static_config[glb_arr_idx].phy_id = phy_info.phy_addr;
            if(bcm_static_config) {
                PHYMOD_IF_MEMCPY(bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config, bcm_static_config, sizeof(plp_static_config_t));
            } 
            break;
        } else if (bcm_plp_aperta2_phy_static_config[glb_arr_idx].phy_id == phy_info.phy_addr) {
            if(bcm_static_config) {
                PHYMOD_IF_MEMCPY(bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config, bcm_static_config, sizeof(plp_static_config_t));
            } 
            break;
 ...
 */
int
bcm_plp_aperta2_static_config_get(bcm_plp_access_t phy_info, void* bcm_static_config)
{
    unsigned int glb_arr_idx = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    for(glb_arr_idx = 0; glb_arr_idx < PHYMOD_IF_CONFIG_MAX_PHYS; glb_arr_idx++) {
        if(phy_info.phy_addr == bcm_plp_aperta2_phy_static_config[glb_arr_idx].phy_id) {
            if (bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config) {
                PHYMOD_IF_MEMCPY(bcm_static_config, bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config, sizeof(plp_static_config_t));
            } else {
                PHYMOD_IF_MEMSET(bcm_static_config, 0, sizeof(plp_static_config_t));
            }
            break;
 ...
 */
int
bcm_plp_aperta2_init(bcm_plp_access_t phy_info,   
...
{
    plp_aperta2_phymod_bus_t core_bus;
    ...
    plp_aperta2_phymod_access_t *pm_acc = NULL;
    int rv = BCM_PM_IF_SUCCESS;
...
#endif
    plp_aperta2_phymod_dispatch_type_t phy_type = 0xFFFF;
...
#endif
    plp_aperta2_phymod_diag_print_func = printf ;

    BCM_PM_MUTEX_LOCK(phy_info.phy_addr, phy_info.platform_ctxt);

    _bcm_plp_aperta2_pm_if_get_phy_id_idx(phy_info.phy_addr, &phy_id_idx, &is_exist_phy);
    if (is_exist_phy == 1) {
 ...
 */
    ...
    if (phy_type == 0xFFFF) {
        ...
        switch(id0) {
            ...
            default:
                ...
                _plp_aperta2_phyid_list[phy_info.phy_addr].valid = 0;
                goto ERR;
        }
    } 

    /* coverity[dead_error_begin:FALSE] */
    rv = plp_aperta2_phymod_bus_t_init(&core_bus);
    if (rv != 0) {
        goto ERR;
    }
    ...
    _bcm_plp_aperta2_pm_if_core_init(&core_probe, &core_bus,
                           phy_info.phy_addr, phy_info.platform_ctxt);
    pm_core = &core_probe.pm_core;
    pm_core->type = phy_type; 

    rv = plp_aperta2_phymod_core_identify(pm_core, 0 , &chip_found);
    if (rv != 0) {
    ...
    if ((plp_aperta2_phy_ctrl.phy[phy_id_idx] == NULL) || (plp_aperta2_phy_ctrl.phy[phy_id_idx]->valid != 1)) { 
        rv = _bcm_plp_aperta2_if_phymod_phy_create(&plp_aperta2_phy_ctrl.phy[phy_id_idx]);
        if (rv != 0) {
            goto ERR;
        }
    }
    pm_phy = &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy;
    plp_aperta2_phymod_phy_access_t_init(pm_phy);

    /*create associated core object */
    rv = _bcm_plp_aperta2_if_phymod_core_create(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core);
    if (rv != 0) {
        goto ERR;
    }
    PHYMOD_IF_MEMSET(plp_aperta2_phy_ctrl.phy[phy_id_idx]->core,0,sizeof(_bcm_if_phymod_core_t));
    core = plp_aperta2_phy_ctrl.phy[phy_id_idx]->core;
    /* Initialize core object if newly created */
    if (core->ref_cnt == 0) {
        PHYMOD_IF_MEMCPY(&core->pm_bus,
                &core_bus, sizeof(core->pm_bus));
        _bcm_plp_aperta2_pm_if_core_init(core, &core->pm_bus,
                           phy_info.phy_addr, phy_info.platform_ctxt);
         /* Set dispatch type */
    ...
    pm_acc = &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access;
    PHYMOD_IF_MEMCPY(pm_acc, &core->pm_core.access, sizeof(*pm_acc));
    plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.type = phy_type;
    PHYMOD_ACC_LANE_MASK(pm_acc) = lane_map;
    phy = plp_aperta2_phy_ctrl.phy[phy_id_idx];
    ...
    if(glb_arr_idx < PHYMOD_IF_CONFIG_MAX_PHYS) {
        if (bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config != NULL) {
            phy->static_config = bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config;
        } else {
    ...
    if (warm_boot_enable) { /* Warm boot environment variable is set to 1 */
        ...
        rv = _bcm_plp_aperta2_pm_phy_init(phy, firmware_load_method, NULL);
        if (rv != 0) {
            PHYMOD_DEBUG_ERROR(("_bcm_plp_aperta2_pm_phy_init rv:%d\n",rv));
            goto ERR;
        }
    }
#else 
        ...
        if (phy_info.flags & BCM_PLP_WARM_BOOT) { /* Warm boot flag is set */
        ...
        #ifdef BCM_PLP_MAC_SUPPORT
            {
                ...
                rv = bcm_plp_aperta2_warmboot_init(mac_access);
                if (rv != 0) {
            ...
            {
                int  dummy = 0;  /* dummy value for the 2nd argument of plp_aperta2_phymod_warmboot_init *\
                                 \*       since PHYMOD doesn't allow null pointer arguments  */
                core->pm_core.access.lane_mask = lane_map;
                rv = plp_aperta2_phymod_warmboot_init(&(core->pm_core), &dummy);
                if ( rv != 0 ) {
                    PHYMOD_DEBUG_ERROR(("WARM BOOT : plp_aperta2_phymod_warmboot_init rv:%d\n",rv));
                    goto ERR;
                }
            }
        #endif
 ...
 */
int
bcm_plp_aperta2_init_fw_bcast(bcm_plp_access_t phy_info,
                  int (*read)(void* user_acc, unsigned int core_addr, unsigned int reg_addr, unsigned int* val),
                  int (*write)(void* user_acc, unsigned int core_addr, unsigned int reg_addr, unsigned int val),
                  ...
                  bcm_pm_firmware_broadcast_method_t broadcast_method)
{
    ...
    BCM_PM_MUTEX_LOCK(phy_info.phy_addr, phy_info.platform_ctxt);
        ...
        if (        bcmpmFirmwareBroadcastCoreReset == broadcast_method ) {  /* 1st step of init */
            _bcm_pm_if_init_phy_id_idx(phy_info.phy_addr);      /* init the _plp_aperta2_phyid_list[] entry  */
            if ( BCM_PHY_IS_MASTER_PORT(phy_addr_combo) ) {       /* MASTER port in 1st init step  */
                ...
                /* mark MASTER flag for the _plp_aperta2_phyid_list[] entry */
    ...
    _bcm_plp_aperta2_pm_if_get_phy_id_idx(phy_addr_combo   , &phy_id_idx, &is_exist_phy);
    ...
    if (!is_exist_phy) {
        ...
        BCM_PM_IF_ERR_RETURN(
                bcm_plp_aperta2_phy_get_phy_type (phy_info, &phy_type, read, write));
        if (phy_type == 0xFFFF) {
...
#if defined(PHYMOD_XGBASET_SUPPORT) || defined(PHYMOD_PHY848XX_SUPPORT)
    /* pass the MASTER port address by plp_aperta2_phymod_core_init_config_s structure */
    ...
    if (warm_boot_enable) { /* Warm boot environment variable is set to 1 */
        ...
        if (phy_info.flags & BCM_PLP_WARM_BOOT) { /* Warm boot flag is set */
            ...
            defined(PHYMOD_MILLENIO_SUPPORT) || defined(PHYMOD_BARCHETTA2_SUPPORT) || defined (PHYMOD_AGERA_SUPPORT) || defined (PHYMOD_AGERA2_SUPPORT) || defined (PHYMOD_AGERALITE_SUPPORT)
            {
                int  dummy = 0;  /* dummy value for the 2nd argument of plp_aperta2_phymod_warmboot_init *\
                                 \*        since PHYMOD doesn't allow null pointer arguments */
                rv = plp_aperta2_phymod_warmboot_init(&(phy->core->pm_core), &dummy);
                if ( rv != 0 ) {
                    PHYMOD_DEBUG_ERROR(("WARM BOOT : plp_aperta2_phymod_warmboot_init rv:%d\n",rv));
                    goto ERR;
                }
            ...
            if (firmware_load_type->force_load_method == bcmpmFirmwareLoadEEPROMUpgrade) {
                PHYMOD_CORE_INIT_F_FW_EEPROM_UPGRADE_SET(&(phy->core->init_config));
                rv = _bcm_plp_aperta2_phy_init_bcast(phy, firmware_load_type);
                if (rv != 0) {
                    PHYMOD_DEBUG_ERROR(("_bcm_plp_aperta2_phy_init_bcast rv:%d\n",rv));
                    goto ERR;
                }
...
COLDBOOT:
    switch(broadcast_method){
        case bcmpmFirmwareBroadcastCoreReset:
            ...
            rv = _bcm_plp_aperta2_phy_init_bcast(phy, firmware_load_type);
            if (rv != 0) {
                PHYMOD_DEBUG_ERROR(("_bcm_plp_aperta2_phy_init_bcast() ERROR!! bcast_method=%d  rv=%d\n", broadcast_method, rv));
                goto ERR;
            }
...
#endif
            rv = _bcm_plp_aperta2_pm_phy_init(phy, firmware_load_type->firmware_load_method, firmware_load_type->fw_init_params);
            if (rv != 0) {
                PHYMOD_DEBUG_ERROR(("_bcm_plp_aperta2_pm_phy_init rv:%d\n",rv));
                goto ERR;
            }
...
ERR:
    ...
    if (rv != 0) {
        ...
        bcm_plp_aperta2_cleanup(phy_info);
    }
 ...
 */
int
bcm_plp_aperta2_cleanup(bcm_plp_access_t phy_info)
{
    uint32_t phy_id_idx;
    ...
    if (phy_info.phy_addr >= BCM_PM_IF_MAX_PHY) {
        PHYMOD_DEBUG_ERROR(("bcm_plp_aperta2_cleanup(): Invalid PHY\n"))
        return BCM_PM_IF_INVALID_PHY;
    }

    _bcm_plp_aperta2_pm_if_get_phy_id_idx(phy_info.phy_addr, &phy_id_idx, &exist_phy);
    if (exist_phy != 1) {
        _plp_aperta2_phyid_list[phy_info.phy_addr].valid = 0;
    ...
    if (plp_aperta2_phy_ctrl.phy[phy_id_idx] && plp_aperta2_phy_ctrl.phy[phy_id_idx]->static_config) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->static_config = NULL;
    }
#ifdef PHYMOD_PHY542XX_SUPPORT
    if (plp_aperta2_phy_ctrl.phy[phy_id_idx]) {
        BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
        /* Call internal driver cleanup function */
        rv = plp_aperta2_phymod_phy_cleanup(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy);
        if(rv){
    ...
    for(glb_arr_idx = 0; glb_arr_idx < PHYMOD_IF_CONFIG_MAX_PHYS; glb_arr_idx++) {
        if((phy_info.phy_addr == bcm_plp_aperta2_phy_static_config[glb_arr_idx].phy_id) &&
           (bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config != NULL)) {
            PHYMOD_IF_FREE(bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config);
            bcm_plp_aperta2_phy_static_config[glb_arr_idx].phy_id = 0xff;
            bcm_plp_aperta2_phy_static_config[glb_arr_idx].bcm_static_config = NULL;
        }
    }
    if (plp_aperta2_phy_ctrl.phy[phy_id_idx]) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->mutex_info.mutex_give = NULL;
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->mutex_info.mutex_take = NULL;
        if (plp_aperta2_phy_ctrl.phy[phy_id_idx]->core) {
            PHYMOD_IF_FREE(plp_aperta2_phy_ctrl.phy[phy_id_idx]->core);
        }
        PHYMOD_IF_FREE(plp_aperta2_phy_ctrl.phy[phy_id_idx]);
        plp_aperta2_phy_ctrl.phy[phy_id_idx] = NULL;
    }
    _plp_aperta2_phyid_list[phy_id_idx].phy_id = 0;
    _plp_aperta2_phyid_list[phy_id_idx].valid = 0;
...
int
bcm_plp_aperta2_link_status_get(bcm_plp_access_t phy_info,  unsigned int *link_status)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_phy_link_status_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, link_status);
 ...
 */
int
bcm_plp_aperta2_mode_config_set(bcm_plp_access_t phy_info,  int speed, int if_type,
                    int ref_clk, int interface_mode, void* device_aux_modes)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0, flags = 0;
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_phy_inf_config_t mode_config;
    ...
    if (aux_mode->pass_thru) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.device_op_mode = 0x0;
    } else {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.device_op_mode = 0x1;
    }
#endif

    PHYMOD_IF_MEMSET(&mode_config, 0, sizeof(plp_aperta2_phymod_phy_inf_config_t));
    ...
    if (speed >= 100000) {
        if ((!plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.device_op_mode) ||
            (plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.port_loc == phymodPortLocSys)) { /* 100G Pass-through */
            if (plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.lane_mask == 0x3ff) {
                interface_mode |= bcmplpInterfaceModeTRIPLECORE;
                interface_mode |= bcmplpInterfaceModeTC442;
            } else if (plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.lane_mask == 0x7fe) {
                interface_mode |= bcmplpInterfaceModeTRIPLECORE;
                interface_mode |= bcmplpInterfaceModeTC343;
            } else if (plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.lane_mask == 0xffc) {
                interface_mode |= bcmplpInterfaceModeTRIPLECORE;
...
#endif
    rv = plp_aperta2_phymod_phy_interface_config_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags, &mode_config);
 ...
 */
int
bcm_plp_aperta2_mode_config_get(bcm_plp_access_t phy_info, int *speed, int *if_type,
                    int *ref_clk, int *interface_mode, void *device_aux_modes)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    ...
    mode_config.device_aux_modes = device_aux_modes;
#ifdef BCM_PLP_BASE_T_PHY
    rv = plp_aperta2_phymod_phy_interface_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
...
#else
    /* coverity[unreachable] */
    if (plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.device_op_mode == 0) {
 ...
 */
void
bcm_plp_aperta2_version_get(unsigned short *chip_ver, unsigned short *api_ver, unsigned short *enahan_ver)
{
 ...
 */
int
bcm_plp_aperta2_prbs_set(bcm_plp_access_t phy_info,  unsigned int tx_rx, 
               unsigned int poly, unsigned int invert, unsigned int loopback, unsigned int ena_dis)
{
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_diag_prbs_set_args_t params;
    uint32_t phy_id_idx = 0xff;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_diag_prbs_args_t prbs_diag_args;
    ...
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
#ifdef PHYMOD_ESTOQUE_SUPPORT
    BCM_PLP_DATA_PATH_DIRECTION(phy_info, plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy);
#endif
    
    prbs_diag_args.prbs_cmd = PhymodDiagPrbsSet;
    PHYMOD_IF_MEMCPY(&prbs_diag_args.args.set_params, &params, sizeof(plp_aperta2_phymod_diag_prbs_set_args_t));

    rv = plp_aperta2_phymod_diag_prbs(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, 1, &prbs_diag_args);
#ifdef PHYMOD_ESTOQUE_SUPPORT
    BCM_PLP_DATA_PATH_DIRECTION_CLR(plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy);
#endif
 ...
 */
int
bcm_plp_aperta2_prbs_get(bcm_plp_access_t phy_info,  unsigned int tx_rx, 
               unsigned int *poly, unsigned int *invert, unsigned int *loopback, unsigned int *ena_dis)
{
    int rv = BCM_PM_IF_SUCCESS;
    ...
    plp_aperta2_phymod_prbs_t prbs_conf_param;
    ...
    rv = plp_aperta2_phymod_phy_prbs_enable_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, tx_rx, &enable);
    if (rv != 0) {
        goto ERR;
    } 
    *ena_dis = enable;
    rv = plp_aperta2_phymod_phy_prbs_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, tx_rx, &prbs_conf_param);
    if (rv != 0) {
        goto ERR;
    } 
 ...
 */
int
bcm_plp_aperta2_prbs_rx_stat(bcm_plp_access_t phy_info,  unsigned int time)
{
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_diag_prbs_get_args_t params;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_diag_prbs_args_t prbs_diag_args;
    ...
    PHYMOD_IF_MEMCPY(&prbs_diag_args.args.get_params, &params, sizeof(plp_aperta2_phymod_diag_prbs_get_args_t));
 ...
 */
int
bcm_plp_aperta2_prbs_clear(bcm_plp_access_t phy_info,  unsigned int tx_rx)
{
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_diag_prbs_clear_args_t params;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_diag_prbs_args_t prbs_diag_args;
    ...
    PHYMOD_IF_MEMCPY(&prbs_diag_args.args.clear_params, &params, sizeof(plp_aperta2_phymod_diag_prbs_clear_args_t));
    rv = plp_aperta2_phymod_diag_prbs(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, 1, &prbs_diag_args);
#ifdef PHYMOD_ESTOQUE_SUPPORT
    BCM_PLP_DATA_PATH_DIRECTION_CLR(plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy);
#endif
 ...
 */
int 
bcm_plp_aperta2_prbs_config_get(bcm_plp_access_t phy_info,  unsigned int tx_rx, unsigned int *poly, unsigned int *invert)
{
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_prbs_t prbs_conf_param;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
...
int
bcm_plp_aperta2_prbs_status_get(bcm_plp_access_t phy_info,  unsigned int *prbs_lock, unsigned int *prbs_lock_loss, unsigned int *error_count)
{
    int rv = BCM_PM_IF_SUCCESS;
    ...
    BCM_PLP_DATA_PATH_DIRECTION(phy_info, plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy);
#endif
    rv = plp_aperta2_phymod_phy_prbs_status_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, 0, &prbs_status);
 ...
 */
int
bcm_plp_aperta2_reg_value_set(bcm_plp_access_t phy_info,  unsigned int devaddr, unsigned int regaddr, unsigned int data)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.devad = devaddr;
    ...
    if (phy_info.data_path_dir == bcmplpDataPathDirectionEgress) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= ((bcmplpDataPathDirectionEgress) << 28);
    }
    if (phy_info.data_path_dir == bcmplpDataPathDirectionIngress) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= ((bcmplpDataPathDirectionIngress) << 28);
    }
#endif
#if defined(PHYMOD_SEAHAWKS_SUPPORT)
		if (phy_info.flags & BCM_PLP_SGMII_REG_ACCESS_MODE) {
			plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_SGMII_REG_ACCESS_MODE;
		}
#endif

    rv = plp_aperta2_phymod_phy_reg_write(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, regaddr, data);
 ...
 */
int
bcm_plp_aperta2_reg_value_get(bcm_plp_access_t phy_info,  unsigned int devaddr, unsigned int regaddr, unsigned int *data)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_reg_read(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, regaddr, data);
 ...
 */
int
bcm_plp_aperta2_polarity_set(bcm_plp_access_t phy_info,  unsigned int tx_pol, unsigned int rx_pol)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_polarity_t phymod_polarity;
    ...
    rv = plp_aperta2_phymod_phy_polarity_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phymod_polarity); 
 ...
 */
int
bcm_plp_aperta2_polarity_get(bcm_plp_access_t phy_info,  unsigned int *tx_pol, unsigned int *rx_pol)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_polarity_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phymod_polarity);
 ...
 */
int
bcm_plp_aperta2_rx_pmd_lock_get(bcm_plp_access_t phy_info,  unsigned int* rx_pmd_locked)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv =  plp_aperta2_phymod_phy_rx_pmd_locked_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &rx_pmd_lock);
 ...
 */
int
bcm_plp_aperta2_power_set(bcm_plp_access_t phy_info,  unsigned int power_rx, unsigned int power_tx)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_phy_power_t phymod_power;
    ...
    rv = plp_aperta2_phymod_phy_power_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phymod_power);
 ...
 */
int
bcm_plp_aperta2_power_get(bcm_plp_access_t phy_info,  unsigned int *power_rx, unsigned int *power_tx)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_power_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phymod_power);
 ...
 */
int
bcm_plp_aperta2_rev_id(bcm_plp_access_t phy_info,  unsigned int* rev_id)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv =  plp_aperta2_phymod_phy_rev_id(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &chip_rev_id);
 ...
 */
int
bcm_plp_aperta2_loopback_set(bcm_plp_access_t phy_info, unsigned int lb_mode, unsigned int enable)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_loopback_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, (plp_aperta2_phymod_loopback_mode_t)lb_mode, enable);
 ...
 */
int
bcm_plp_aperta2_loopback_get(bcm_plp_access_t phy_info,  unsigned int lb_mode, unsigned int *enable)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    *enable = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_loopback_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, (plp_aperta2_phymod_loopback_mode_t) lb_mode, enable);
 ...
 */
int 
bcm_plp_aperta2_tx_set(bcm_plp_access_t phy_info,  bcm_plp_tx_t* tx)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_tx_t tx_param;
    PHYMOD_MEMSET(&tx_param, 0, sizeof(plp_aperta2_phymod_tx_t));
    tx_param.pre = tx->pre;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_tx_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &tx_param);    
 ...
 */
int 
bcm_plp_aperta2_tx_get(bcm_plp_access_t phy_info,  bcm_plp_tx_t* tx)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_tx_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &tx_param); 
 ...
 */
int 
bcm_plp_aperta2_pam4_tx_set(bcm_plp_access_t phy_info,  bcm_plp_pam4_tx_t* tx)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_pam4_tx_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &tx_param); 
    if (rv != PHYMOD_E_NONE) {
        goto ERR;
    }
#if defined (PHYMOD_APERTA2_SUPPORT)
    {
        plp_aperta2_phymod_tx_override_t tx_pi_freq_override;
        PHYMOD_MEMCPY(&tx_pi_freq_override.phase_interpolator, &tx->tx_pi_freq_override, sizeof(bcm_plp_value_override_t)); 
        rv = plp_aperta2_phymod_phy_tx_override_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &tx_pi_freq_override);
    }
#endif
 ...
 */
int 
bcm_plp_aperta2_pam4_tx_get(bcm_plp_access_t phy_info,  bcm_plp_pam4_tx_t* tx)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    /* coverity[mixed_enums] */
    tx_param.serdes_tx_tap_mode = tx->serdes_tx_tap_mode;
    rv = plp_aperta2_phymod_phy_pam4_tx_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &tx_param); 
    if (rv != PHYMOD_E_NONE) {
        goto ERR;
    }
    ...
    {
        plp_aperta2_phymod_tx_override_t tx_pi_freq_override;
        rv = plp_aperta2_phymod_phy_tx_override_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &tx_pi_freq_override);
 ...
 */
int 
bcm_plp_aperta2_rx_set(bcm_plp_access_t phy_info,  bcm_plp_rx_t* rx)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    uint32_t index = 0;
    plp_aperta2_phymod_rx_t rx_param;
    PHYMOD_IF_MEMSET(&rx_param, 0, sizeof(plp_aperta2_phymod_rx_t));
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_rx_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &rx_param);    
 ...
 */
int 
bcm_plp_aperta2_rx_get(bcm_plp_access_t phy_info,  bcm_plp_rx_t* rx)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    PHYMOD_IF_MEMSET(&rx_param, 0, sizeof(plp_aperta2_phymod_rx_t));
    rv = plp_aperta2_phymod_phy_rx_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &rx_param);   
    if(!rv) {
 ...
 */
int 
bcm_plp_aperta2_reset_set(bcm_plp_access_t phy_info,  unsigned int reset_mode, unsigned int reset_direction)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_core_reset_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, reset_mode, reset_direction); 
 ...
 */
int
bcm_plp_aperta2_phy_lane_reset_set(bcm_plp_access_t phy_info,  bcm_plp_pm_phy_reset_t* reset)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_phy_reset_t rst;
    rst.tx = reset->tx; 
    rst.rx = reset->rx; 
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_reset_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &rst);
 ...
 */
int
bcm_plp_aperta2_phy_lane_reset_get(bcm_plp_access_t phy_info,  bcm_plp_pm_phy_reset_t* reset)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_reset_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &rst);
    reset->tx = rst.tx;
    reset->rx = rst.rx;
 ...
 */
int 
bcm_plp_aperta2_tx_lane_control_set(bcm_plp_access_t phy_info,  bcm_pm_phy_tx_lane_control_t tx_control)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    /* coverity[mixed_enums] */
    rv = plp_aperta2_phymod_phy_tx_lane_control_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, tx_control);    
...
int
bcm_plp_aperta2_tx_lane_control_get(bcm_plp_access_t phy_info,  bcm_pm_phy_tx_lane_control_t *tx_control)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_phy_tx_lane_control_t tx_ctrl = 0;
    ...
    /* coverity[mixed_enums] */
    tx_ctrl = *tx_control;
    rv = plp_aperta2_phymod_phy_tx_lane_control_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &tx_ctrl);
    /* coverity[mixed_enums] */
    *tx_control = tx_ctrl;
 ...
 */
int 
bcm_plp_aperta2_rx_lane_control_set(bcm_plp_access_t phy_info,  bcm_pm_phy_rx_lane_control_t rx_control)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_rx_lane_control_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, rx_control);    
...
int
bcm_plp_aperta2_rx_lane_control_get(bcm_plp_access_t phy_info,  bcm_pm_phy_rx_lane_control_t *rx_control)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_phy_rx_lane_control_t rx_ctrl = 0;
    ...
    rx_ctrl = *rx_control;
    rv = plp_aperta2_phymod_phy_rx_lane_control_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &rx_ctrl);
    *rx_control = rx_ctrl;
 ...
 */
int 
bcm_plp_aperta2_lane_cross_switch_map_set(bcm_plp_access_t phy_info, unsigned int* tx_source_array)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_lane_cross_switch_map_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, tx_source_array);    
 ...
 */
int 
bcm_plp_aperta2_lane_cross_switch_map_get(bcm_plp_access_t phy_info,  unsigned int* tx_source_array)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_lane_cross_switch_map_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, tx_source_array);    
...
{
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_autoneg_ability_t  pm_ability;
    ...
    PHYMOD_IF_MEMSET(&pm_ability, 0, sizeof(plp_aperta2_phymod_autoneg_ability_t));
    ...
    rv = plp_aperta2_phymod_phy_autoneg_ability_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &pm_ability);
...
bcm_base_t_autoneg_ability_get(bcm_plp_access_t phy_info, bcm_plp_base_t_ability_t *ability)
{
    ...
    if (ability->an_side == 1) {
        rv = plp_aperta2_phymod_phy_autoneg_remote_ability_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &pm_ability);
    } else {
        rv = plp_aperta2_phymod_phy_autoneg_ability_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &pm_ability);
    }
...
{
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_autoneg_control_t  an_ctrl;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY, exist_phy = 0;
    ...
    PHYMOD_MEMSET(&an_ctrl, 0, sizeof(plp_aperta2_phymod_autoneg_control_t));
    an_ctrl.enable = enable; 
    
    rv = plp_aperta2_phymod_phy_autoneg_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &an_ctrl);
...
{
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_autoneg_status_t  an_ststus;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY, exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_autoneg_status_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &an_ststus);
 ...
 */
int
bcm_plp_aperta2_cl73_ability_set(bcm_plp_access_t phy_info, unsigned short tech_ability,
		           unsigned short fec_ability, unsigned short pause_ability, bcm_plp_an_config_t an_config)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_autoneg_ability_t ability;
    ...
    PHYMOD_IF_ERR_RETURN(_bcm_plp_aperta2_phy_user_phymod_ability((tech_ability | (an_config.tech_ability)), fec_ability, pause_ability, an_config, &ability));
#else
    PHYMOD_IF_ERR_RETURN(_bcm_plp_aperta2_phy_user_phymod_ability(tech_ability, fec_ability, pause_ability, an_config, &ability));
#endif
    ...
    rv = plp_aperta2_phymod_phy_autoneg_ability_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &ability);
 ...
 */
int
bcm_plp_aperta2_cl73_ability_get(bcm_plp_access_t phy_info, unsigned short *tech_ability,
		           unsigned short *fec_ability, unsigned short *pause_ability, bcm_plp_an_config_t* an_config)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    ...
    rv = plp_aperta2_phymod_phy_autoneg_ability_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &ability);
    PHYMOD_IF_ERR_RETURN(_bcm_plp_aperta2_phy_phymod_user_ability(ability, fec_ability, pause_ability, an_config));
 ...
 */
int
bcm_plp_aperta2_autoneg_remote_ability_get(bcm_plp_access_t phy_info, unsigned short *fec_ability, 
                               unsigned short *pause_ability, bcm_plp_an_config_t* an_config)
{
    ...
    rv = plp_aperta2_phymod_phy_autoneg_remote_ability_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &ability);
    PHYMOD_IF_ERR_RETURN(_bcm_plp_aperta2_phy_phymod_user_ability(ability, fec_ability, pause_ability, an_config));
 ...
 */
int
bcm_plp_aperta2_cl73_set(bcm_plp_access_t phy_info, unsigned short ena_dis)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    if (ena_dis == 2) { /* AN Restart */
        an.enable = 1;
        rv = plp_aperta2_phymod_phy_autoneg_restart_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &an);
    } else {
#endif
    rv = plp_aperta2_phymod_phy_autoneg_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &an);
#if defined(PHYMOD_MILLENIO_SUPPORT) || defined(PHYMOD_BARCHETTA2_SUPPORT)
    }
#endif
 ...
 */
int
bcm_plp_aperta2_cl73_get(bcm_plp_access_t phy_info,  unsigned int *an, unsigned int *an_done)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_autoneg_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &an_ctrl, an_done);
    *an = an_ctrl.enable;
 ...
 *  ***************************************************************************\n
 */
int bcm_plp_aperta2_display_eye_scan(bcm_plp_access_t phy_info)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_eyescan_run(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,flags,mode,0);
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_firmware_info_get(bcm_plp_access_t phy_info,  unsigned int *fw_version, unsigned int *fw_crc)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_core_firmware_info_t firmware_info;

    BCM_PM_MUTEX_LOCK(phy_info.phy_addr, phy_info.platform_ctxt);
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_CORE_ACCESS(phy_info, phy_id_idx);

    rv = plp_aperta2_phymod_core_firmware_info_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, &firmware_info);
 ...
 */
int
bcm_plp_aperta2_firmware_set(bcm_plp_access_t phy_info)
{
    int rv = BCM_PM_IF_INVALID_PHY;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_core_init_config_t  init_config;
    plp_aperta2_phymod_core_status_t       core_sts;
 
    BCM_PM_MUTEX_LOCK(phy_info.phy_addr, phy_info.platform_ctxt);
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_CORE_ACCESS(phy_info, phy_id_idx);
    PHYMOD_MEMSET(&init_config, 0, sizeof(plp_aperta2_phymod_core_init_config_t));
    PHYMOD_MEMSET(&core_sts   , 0, sizeof(plp_aperta2_phymod_core_status_t));
    ...
    rv = plp_aperta2_phymod_core_init(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, &init_config, &core_sts);
 ...
 */
int 
bcm_plp_aperta2_rxtx_laneswap_set(bcm_plp_access_t phy_info, bcm_laneswap_map_t *laneswap_map)
{
    int      ii, rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_lane_map_t map_param;
    ...
    rv = plp_aperta2_phymod_core_lane_map_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, &map_param);    
 ...
 */
int 
bcm_plp_aperta2_rxtx_laneswap_get(bcm_plp_access_t phy_info, bcm_laneswap_map_t* laneswap_map)
{
    ...
    rv = plp_aperta2_phymod_core_lane_map_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, &map_param);    
    if ( 0 == rv ) {
        for ( ii = 0; ii < map_param.num_of_lanes; ii++ ) {
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_pll_sequencer_restart(bcm_plp_access_t phy_info,  unsigned char flags, bcm_pm_sequencer_operation_t operation)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_CORE_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_core_pll_sequencer_restart(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, flags, operation);
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_fec_enable_set(bcm_plp_access_t phy_info,  unsigned int enable)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_fec_enable_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, enable);
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_fec_enable_get(bcm_plp_access_t phy_info,  unsigned int* enable)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_fec_enable_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, enable);
...
int bcm_plp_aperta2_jfec_config_set(bcm_plp_access_t phy_info, bcm_plp_jfec_config_t jfec_config)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_jfec_config_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, phy_jfec_config);
#ifdef PHYMOD_ESTOQUE_SUPPORT
    BCM_PLP_DATA_PATH_DIRECTION_CLR(plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy);
#endif
ERR:
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_jfec_config_get(bcm_plp_access_t phy_info, bcm_plp_jfec_config_t *jfec_config)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_jfec_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phy_jfec_config);
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_kp4_fec_config_set(bcm_plp_access_t phy_info, bcm_plp_kp4_fec_config_t kp4_fec_config)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_kp4_config_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, phy_kp4_fec_config);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_kp4_fec_config_get(bcm_plp_access_t phy_info, bcm_plp_kp4_fec_config_t *kp4_fec_config)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_kp4_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phy_kp4_fec_config);
...
int bcm_plp_aperta2_fec_status_get(bcm_plp_access_t phy_info, bcm_plp_fec_status_t *fec_status)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    ...
    rv = plp_aperta2_phymod_phy_pam4_fec_status_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phy_fec_dump_sts);
    /* coverity[mixed_enums] */
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_phy_status_dump(bcm_plp_access_t phy_info)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_phy_diagnostics_t phy_diag;
    PHYMOD_IF_MEMSET(&phy_diag, 0, sizeof(plp_aperta2_phymod_phy_diagnostics_t));
    ...
    if (phy_info.flags & BCM_PLP_INTERNAL_DUMP) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_INTERNAL_DUMP;
    }
    if (phy_info.flags & BCM_PLP_CUSTOM_DIAG_DUMP) {
#if defined(PHYMOD_XGBASET_SUPPORT)
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_CUSTOM_DIAG_DUMP;
#endif
        plp_aperta2_phymod_custom_diag_dump_hdr(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy);
        rv = plp_aperta2_phymod_phy_diagnostics_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phy_diag);
#if defined(PHYMOD_XGBASET_SUPPORT)
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags &= (~BCM_PLP_CUSTOM_DIAG_DUMP);
#endif
        if(!rv) {
            plp_aperta2_phymod_custom_diag_dump_display(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phy_diag);
        }
        goto ERR;
    }
#if defined(PHYMOD_ESTOQUE_SUPPORT) || defined(PHYMOD_MILLENIO_SUPPORT)
    if (phy_info.flags & BCM_PLP_INTERNAL_CHIP_DUMP) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_INTERNAL_CHIP_DUMP;
    }
    if (phy_info.flags & BCM_PLP_INTERNAL_CORE_LANE_DUMP) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_INTERNAL_CORE_LANE_DUMP;
    }
    if (phy_info.flags & BCM_PLP_INTERNAL_CHIP_DSC_DUMP) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_INTERNAL_CHIP_DSC_DUMP;
    }
    if (phy_info.flags & BCM_PLP_INTERNAL_DIAG_REG_DUMP) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_INTERNAL_DIAG_REG_DUMP;
    }
    if (phy_info.flags & BCM_PLP_INTERNAL_EVENT_UC_DUMP) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_INTERNAL_EVENT_UC_DUMP;
    }
#endif
    ...
    if (phy_info.flags & BCM_PLP_INTERNAL_DUMP_L1) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_INTERNAL_DUMP_L1;
    }
    if (phy_info.flags & BCM_PLP_INTERNAL_DUMP_L2) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_INTERNAL_DUMP_L2;
    }
    if (phy_info.flags & BCM_PLP_INTERNAL_DUMP_L3) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags |= BCM_PLP_INTERNAL_DUMP_L3;
    }
#endif
    rv = plp_aperta2_phymod_phy_status_dump(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy);
    if (phy_info.flags & BCM_PLP_INTERNAL_DUMP) {
        plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.flags &= (~BCM_PLP_INTERNAL_DUMP);
    }
#if defined(PHYMOD_ESTOQUE_SUPPORT) || defined(PHYMOD_MILLENIO_SUPPORT)
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_phy_diagnostics_get(bcm_plp_access_t phy_info,  bcm_plp_pm_phy_diagnostics_t* diag)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    if(!rv) {
        PHYMOD_IF_MEMCPY(diag, &phy_diag, sizeof(plp_aperta2_phymod_phy_diagnostics_t));
    }
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_phy_pam4_diagnostics_get(bcm_plp_access_t phy_info,  bcm_plp_pm_phy_pam4_diagnostics_t* diag)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_phy_pam4_diagnostics_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phy_diag);
    if(!rv) {
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_intr_status_get(bcm_plp_access_t phy_info, unsigned int intr_type, unsigned int* intr_status)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    *intr_status = intr_type;
    ...
    rv = plp_aperta2_phymod_phy_intr_status_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, intr_status);    
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_event_status_get(bcm_plp_access_t phy_info, unsigned int event_type, unsigned int* event_status)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    *event_status = (event_type) | (1 << 31);
    ...
    rv = plp_aperta2_phymod_phy_intr_status_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, event_status);    
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_intr_enable_set(bcm_plp_access_t phy_info, unsigned int intr_type, unsigned int enable)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
...
#endif
    rv = plp_aperta2_phymod_phy_intr_enable_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, temp_enable);    
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_intr_enable_get(bcm_plp_access_t phy_info, unsigned int intr_type, unsigned int* enable)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_intr_enable_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,  enable);    
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_intr_status_clear(bcm_plp_access_t phy_info, unsigned int intr_type)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_intr_status_clear(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, intr_type);    
...
int bcm_base_t_eee_set(bcm_plp_access_t phy_info, bcm_plp_base_t_eee_t *eee_conf)
{
    ...
    rv = plp_aperta2_phymod_phy_eee_mode_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, (phymod_eee_t*) eee_conf);    
...
int bcm_base_t_eee_get(bcm_plp_access_t phy_info, bcm_plp_base_t_eee_t *eee_conf)
{
    ...
    rv = plp_aperta2_phymod_phy_eee_mode_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, (phymod_eee_t*) eee_conf);    
...
int bcm_base_t_power_mode_set(bcm_plp_access_t phy_info, bcm_plp_base_t_power_mode_t power_mode)
{
    ...
    rv = plp_aperta2_phymod_phy_power_mode_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
...
int bcm_base_t_power_mode_get(bcm_plp_access_t phy_info, bcm_plp_base_t_power_mode_t *power_mode)
{
    ...
    rv = plp_aperta2_phymod_phy_power_mode_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
                                   (phymod_power_mode_t*) power_mode);    
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
{
    plp_aperta2_phymod_phy_diagnostics_t  phymod_diag;
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY, exist_phy = 0;
    ...
    PHYMOD_MEMSET(&phymod_diag, 0, sizeof(plp_aperta2_phymod_phy_diagnostics_t));
    phymod_diag.state_mfg_diag_op_cmd = bcmplpBaseTDiagCtrlCmdCableDiag;
    phymod_diag.state_mfg_diag_arg    = cdiag;

    rv = plp_aperta2_phymod_phy_diagnostics_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phymod_diag);    
...
int bcm_plp_aperta2_eye_margin_proj(bcm_plp_access_t phy_info,  double rate, unsigned char ber_scan_mode, unsigned char timer_control, unsigned char max_error_control)
#else
int bcm_plp_aperta2_eye_margin_proj(bcm_plp_access_t phy_info,  int rate, unsigned char ber_scan_mode, unsigned char timer_control, unsigned char max_error_control)
#endif
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    ...
    plp_aperta2_phymod_phy_eyescan_options_t eyescan_options;
    ...
    rv = plp_aperta2_phymod_phy_eyescan_run(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,flags,mode, &eyescan_options);
 ...
 *                     */
int
bcm_plp_aperta2_repeater_mode_set(bcm_plp_access_t phy_info, unsigned int ena_dis)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_interface_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, 0,
           ref_clk,  &mode_config);
    if (rv != 0) {
       PHYMOD_DEBUG_ERROR(("plp_aperta2_phymod_phy_interface_config_get error rv=%d\n",rv));
       goto ERR;
    }
    phy= plp_aperta2_phy_ctrl.phy[phy_id_idx];
    ...
    rv = plp_aperta2_phymod_phy_init(&phy->pm_phy, &phy->init_config);
    if (rv != 0) {
 ...
 *    @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_module_read(bcm_plp_access_t phy_info, unsigned int slv_dev_addr,
                    ...
                    unsigned char *read_data)
{
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_i2c_read(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, unused_flags, slv_dev_addr, start_addr, no_of_bytes, read_data);
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_module_write(bcm_plp_access_t phy_info, unsigned int slv_dev_addr,
                     unsigned int start_addr, unsigned int no_of_bytes,
                     unsigned char *write_data)
{
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_i2c_write(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, unused_flags, slv_dev_addr, start_addr, no_of_bytes, write_data);
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_peripheral_device_control(bcm_plp_access_t phy_info,
                                  ...
                                  bcm_plp_peripheral_dev_ctrl_data_t *ctrl_data)
{
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_peripheral_dev_control(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_cfg_gpio_pin_set(bcm_plp_access_t phy_info, 
                               ...
                               unsigned int cfg_pull, unsigned int pin_value)
{
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_gpio_config_set(
            &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, 
            gpio_pin_number, 
            (plp_aperta2_phymod_gpio_mode_t ) (cfg_direction + 1));

    if (rv) {
        goto ERR;
    }

    rv = plp_aperta2_phymod_phy_gpio_pin_value_set(
            &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, 
            gpio_pin_number, 
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_cfg_gpio_pin_get(bcm_plp_access_t phy_info, 
                               unsigned int gpio_pin_number, unsigned int *cfg_direction,
                               unsigned int *cfg_pull, unsigned int *pin_value)
{
    int rv = BCM_PM_IF_SUCCESS;
    ...
    uint32_t exist_phy = 0;
    int value;
    plp_aperta2_phymod_gpio_mode_t mode;

    BCM_PM_MUTEX_LOCK(phy_info.phy_addr, phy_info.platform_ctxt);
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_gpio_config_get(
            &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
            gpio_pin_number, 
            &mode);
    ...
    *cfg_direction = mode - 1;
    ...
    rv = plp_aperta2_phymod_phy_gpio_pin_value_get(
            &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
            gpio_pin_number, 
            &value);
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_force_tx_training_set(bcm_plp_access_t phy_info, unsigned int enable)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_cl72_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, enable);    
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_force_tx_training_get(bcm_plp_access_t phy_info, unsigned int *enable)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_cl72_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, enable);    
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_force_tx_training_status_get(bcm_plp_access_t phy_info, unsigned int *enable, unsigned int *training_failure, unsigned int *trained)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_cl72_status_t cl72_status;

    BCM_PM_MUTEX_LOCK(phy_info.phy_addr, phy_info.platform_ctxt);
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_cl72_status_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &cl72_status);
 ...
 */
int
bcm_plp_aperta2_firmware_lane_config_set(bcm_plp_access_t phy_info, bcm_plp_pm_firmware_lane_config_t* firmware_lane_config)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_firmware_lane_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &fw_config);
    if (rv) {
        goto ERR;
    }
    ...
    rv = plp_aperta2_phymod_phy_firmware_lane_config_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, fw_config);
 ...
 */
int
bcm_plp_aperta2_firmware_lane_config_get(bcm_plp_access_t phy_info, bcm_plp_pm_firmware_lane_config_t* firmware_lane_config)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_fc_pcs_chkr_enable_set(bcm_plp_access_t phy_info, unsigned int fcpcs_chkr_mode, unsigned int enable)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_link_mon_enable_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, fcpcs_chkr_mode, enable);
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_fc_pcs_chkr_enable_get(bcm_plp_access_t phy_info, unsigned int fcpcs_chkr_mode, unsigned int* enable)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_link_mon_enable_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, fcpcs_chkr_mode, enable);
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_fc_pcs_chkr_status_get(bcm_plp_access_t phy_info, unsigned int *lock_status, unsigned int* lock_lost_lh, unsigned int* error_count)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_link_mon_status_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, lock_status, lock_lost_lh, error_count);
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_failover_mode_set(bcm_plp_access_t phy_info,unsigned int failover_mode)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_failover_mode_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,failover_mode);
...
int bcm_plp_aperta2_failover_mode_get(bcm_plp_access_t phy_info,unsigned int *failover_mode)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_failover_mode_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,failover_mode);
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_failover_config_set(bcm_plp_access_t phy_info, bcm_plp_failover_config_t failover_config)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_failover_config_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, failover_cfg);
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_failover_config_get(bcm_plp_access_t phy_info, bcm_plp_failover_config_t *failover_config)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_failover_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &failover_cfg);
    /* coverity[mixed_enums] */
...
int bcm_plp_aperta2_edc_config_set(bcm_plp_access_t phy_info, unsigned int edc_method, unsigned int edc_value )
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_edc_config_t edc_config;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv=plp_aperta2_phymod_edc_config_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &edc_config);
...
int bcm_plp_aperta2_edc_config_get(bcm_plp_access_t phy_info, unsigned int *edc_method, unsigned int *edc_value )
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv=plp_aperta2_phymod_edc_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &edc_config);
    if(!rv){
 ...
 */
int
bcm_plp_aperta2_driver_version_get(bcm_plp_access_t phy_info, char *phy_chip_name, unsigned short *major_ver, unsigned short *minor_ver)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_core_info_t core_info;

    PHYMOD_IF_MEMSET(&core_info, 0, sizeof(plp_aperta2_phymod_core_info_t));
    BCM_PM_MUTEX_LOCK(phy_info.phy_addr, phy_info.platform_ctxt);
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_CORE_ACCESS(phy_info, phy_id_idx);
    ...
    *minor_ver = BCM_PM_IF_MINOR_VER_NO;
    ...
    rv = plp_aperta2_phymod_core_info_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, &core_info);
    /* coverity[array_null] */
    if ((rv == 0) && (core_info.name != NULL)) {
 ...
 */
int
bcm_plp_aperta2_fec_corrected_error_counter(bcm_plp_access_t phy_info, bcm_plp_fec_type_t fec_type, unsigned int *count)
{
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_phy_fec_correctable_counter_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, pm_fec_type, count);
 ...
 */
int
bcm_plp_aperta2_fec_uncorrected_error_counter(bcm_plp_access_t phy_info, bcm_plp_fec_type_t fec_type, unsigned int *count)
{
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_phy_fec_uncorrectable_counter_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, pm_fec_type, count);
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_config_set(bcm_plp_access_t phy_info,
                                bcm_plp_timesync_config_t* config) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_timesync_config_t phymod_ts_config;

    PHYMOD_IF_MEMSET(&phymod_ts_config, 0, sizeof(plp_aperta2_phymod_timesync_config_t));
    ...
    rv = plp_aperta2_phymod_timesync_config_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phymod_ts_config);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_config_get(bcm_plp_access_t phy_info,
                                bcm_plp_timesync_config_t* config) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_timesync_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phymod_ts_config);
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_enable_set(bcm_plp_access_t phy_info,
    ...
    rv = plp_aperta2_phymod_timesync_enable_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
                                      flags, enable);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_enable_get(bcm_plp_access_t phy_info,
                                unsigned int flags, unsigned int* enable) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_timesync_enable_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
                                      flags, enable);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*! 
 * bcm_plp_aperta2_timesync_nco_addend_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_nco_addend_set(bcm_plp_access_t phy_info,
    ...
    rv = plp_aperta2_phymod_timesync_nco_addend_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, freq_step);
...
/*! 
 * bcm_plp_aperta2_timesync_nco_addend_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_nco_addend_get(bcm_plp_access_t phy_info, unsigned int flags, unsigned int* freq_step) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_timesync_nco_addend_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, freq_step);
...
/*! 
 * bcm_plp_aperta2_timesync_framesync_mode_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_framesync_mode_set(bcm_plp_access_t phy_info, unsigned int flags,
                                    bcm_plp_timesync_framesync_t* framesync) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_timesync_framesync_t phymod_ts_fsync ;

    PHYMOD_IF_MEMSET(&phymod_ts_fsync, 0, sizeof(plp_aperta2_phymod_timesync_framesync_t));

    BCM_PM_MUTEX_LOCK(phy_info.phy_addr, phy_info.platform_ctxt);
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    BCM_PLP_CONVERT_TS_FSYNC((&phymod_ts_fsync), framesync);
    rv = plp_aperta2_phymod_timesync_framesync_mode_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phymod_ts_fsync);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*! 
 * bcm_plp_aperta2_timesync_framesync_mode_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_framesync_mode_get(bcm_plp_access_t phy_info, unsigned int flags,
                                    bcm_plp_timesync_framesync_t* framesync) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_timesync_framesync_mode_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phymod_ts_fsync);
...
/*! 
 * bcm_plp_aperta2_timesync_local_time_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_local_time_set(bcm_plp_access_t phy_info,
    ...
    rv = plp_aperta2_phymod_timesync_local_time_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, local_time);
...
/*! 
 * bcm_plp_aperta2_timesync_local_time_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_local_time_get(bcm_plp_access_t phy_info,
                                unsigned int flags, plp_uint64_t* local_time) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_timesync_local_time_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, local_time);
...
/*! 
 * bcm_plp_aperta2_timesync_load_ctrl_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_load_ctrl_set(bcm_plp_access_t phy_info, unsigned int flags,
    ...
    rv = plp_aperta2_phymod_timesync_load_ctrl_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
...
/*! 
 * bcm_plp_aperta2_timesync_load_ctrl_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_load_ctrl_get(bcm_plp_access_t phy_info, unsigned int flags,
                               unsigned int* load_once, unsigned int* load_always) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_timesync_load_ctrl_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
                                         load_once, load_always);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*! 
 * bcm_plp_aperta2_timesync_timing_control_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_timing_control_set(bcm_plp_access_t phy_info, unsigned int flags,
                                    bcm_plp_timesync_time_value_t *time_ctrl)
{
    ...
    rv = plp_aperta2_phymod_timesync_timing_control_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
...
/*!
 * bcm_plp_aperta2_timesync_timing_control_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_timing_control_get(bcm_plp_access_t phy_info, unsigned int flags,
                                    bcm_plp_timesync_time_value_t *time_ctrl)
{
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_timesync_timing_control_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
                                    time_ctrl->type, &(time_ctrl->time_value),
                                    &time_ctrl->time_value_hi, &time_ctrl->dpll_time_value,
                                    &time_ctrl->dpll_time_value_hi);
ERR:
...
/*!
 * bcm_plp_aperta2_timesync_link_delay_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_link_delay_set(bcm_plp_access_t phy_info, unsigned int flags,
                                bcm_plp_timesync_time_value_t *linkdelay)
{
    ...
    rv = plp_aperta2_phymod_timesync_link_delay_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
...
/*!
 * bcm_plp_aperta2_timesync_link_delay_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_link_delay_get(bcm_plp_access_t phy_info, unsigned int flags,
                                bcm_plp_timesync_time_value_t *linkdelay)
{
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_timesync_link_delay_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
                                          linkdelay->direction, &(linkdelay->op),
                                        &(linkdelay->type)    , &(linkdelay->time_value));
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*!
 * bcm_plp_aperta2_timesync_timestamp_offset_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_timestamp_offset_set(bcm_plp_access_t phy_info, unsigned int flags,
                                      bcm_plp_timesync_time_value_t *ts_offset)
{
    ...
    rv = plp_aperta2_phymod_timesync_timestamp_offset_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
...
/*!
 * bcm_plp_aperta2_timesync_timestamp_offset_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_timestamp_offset_get(bcm_plp_access_t phy_info, unsigned int flags,
                                      bcm_plp_timesync_time_value_t *ts_offset)
{
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_timesync_timestamp_offset_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
                                          ts_offset->direction, &(ts_offset->op),
                                        &(ts_offset->type)    , &(ts_offset->time_value));
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*! 
 * bcm_plp_aperta2_timesync_tx_timestamp_offset_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_tx_timestamp_offset_set(bcm_plp_access_t phy_info,
    ...
    rv = plp_aperta2_phymod_timesync_tx_timestamp_offset_set(
                                &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, ts_offset);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*! 
 * bcm_plp_aperta2_timesync_tx_timestamp_offset_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_tx_timestamp_offset_get(bcm_plp_access_t phy_info,
                                         unsigned int flags, unsigned int* ts_offset) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_timesync_tx_timestamp_offset_get(
                                &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, ts_offset);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*! 
 * bcm_plp_aperta2_timesync_rx_timestamp_offset_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_rx_timestamp_offset_set(bcm_plp_access_t phy_info,
                                         unsigned int flags, unsigned int ts_offset) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_timesync_rx_timestamp_offset_set(
                                &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, ts_offset);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*! 
 * bcm_plp_aperta2_timesync_rx_timestamp_offset_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_rx_timestamp_offset_get(bcm_plp_access_t phy_info,
                                         unsigned int flags, unsigned int* ts_offset) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_timesync_rx_timestamp_offset_get(
                                &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, ts_offset);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*!
 * bcm_plp_aperta2_timesync_time_code_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_time_code_set(bcm_plp_access_t phy_info, unsigned int flags,
                               bcm_plp_timesync_timespec_t *timecode) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy  = 0;
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_timesync_timespec_t tcode;
    ...
    rv = plp_aperta2_phymod_timesync_time_code_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
                                       flags, &tcode);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
    return rv;
...
/*!
 * bcm_plp_aperta2_timesync_time_code_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_time_code_get(bcm_plp_access_t phy_info, unsigned int flags,
                               bcm_plp_timesync_timespec_t *timecode) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy  = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_timesync_time_code_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy,
                                       flags, &tcode);
...
/*!
 * bcm_plp_aperta2_timesync_sopmem_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_sopmem_set(bcm_plp_access_t phy_info, unsigned int flags,
                            int index, bcm_plp_timesync_sopmem_t *sopentry)
{
    ...
    rv = plp_aperta2_phymod_timesync_sopmem_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags,
                                    &sopmem_entry);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*! 
 * bcm_plp_aperta2_timesync_sopmem_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_sopmem_get(bcm_plp_access_t phy_info, unsigned int flags,
                            int index, bcm_plp_timesync_sopmem_t *sopentry)
{
    ...
    rv = plp_aperta2_phymod_timesync_sopmem_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags,
                                    index, &sopmem_entry);
    ...
    rv = plp_aperta2_phymod_timesync_sopmem_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags, index, sopmem_entry);
...
/*!
 * bcm_plp_aperta2_timesync_capture_timestamp_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_capture_timestamp_get(bcm_plp_access_t phy_info,
    ...
    rv = plp_aperta2_phymod_timesync_capture_timestamp_get(
                                &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, cap_ts);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*! 
 * bcm_plp_aperta2_timesync_heartbeat_timestamp_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_heartbeat_timestamp_get(bcm_plp_access_t phy_info,
    ...
    rv = plp_aperta2_phymod_timesync_heartbeat_timestamp_get(
                                &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, hb_ts);
...
/*!
 * bcm_plp_aperta2_timesync_inband_filter_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_inband_filter_set(bcm_plp_access_t phy_info, unsigned int flags,
    ...
    rv = plp_aperta2_phymod_timesync_inband_filter_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags, index, &phymod_ts_ib_flt);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*!
 * bcm_plp_aperta2_timesync_inband_filter_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_inband_filter_get(bcm_plp_access_t phy_info, unsigned int flags,
                                   int index, bcm_plp_timesync_inband_filter_ctrl_t *config) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_timesync_inband_filter_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags, index, &phymod_ts_ib_flt);
...
/*! 
 * bcm_plp_aperta2_timesync_mpls_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_mpls_set(bcm_plp_access_t phy_info, unsigned int flags,
                          int enable, bcm_plp_timesync_mpls_ctrl_t *mpls_ctrl) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_timesync_mpls_ctrl_t phymod_ts_mpls_ctrl;

    PHYMOD_IF_MEMSET(&phymod_ts_mpls_ctrl, 0, sizeof(plp_aperta2_phymod_timesync_mpls_ctrl_t));
    ...
    rv = plp_aperta2_phymod_timesync_mpls_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags, &phymod_ts_mpls_ctrl);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*! 
 * bcm_plp_aperta2_timesync_mpls_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_mpls_get(bcm_plp_access_t phy_info, unsigned int flags,
                          int enable, bcm_plp_timesync_mpls_ctrl_t *mpls_ctrl) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_timesync_mpls_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags, &phymod_ts_mpls_ctrl);
...
/*!
 * bcm_plp_aperta2_timesync_pch_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_pch_set(bcm_plp_access_t phy_info,
    ...
    rv = plp_aperta2_phymod_timesync_pch_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags, &phymod_ts_pch);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*!
 * bcm_plp_aperta2_timesync_pch_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_pch_get(bcm_plp_access_t phy_info,
                         unsigned int flags, bcm_plp_timesync_pch_t *pch) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_timesync_pch_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags, &phymod_ts_pch);
...
/*!
 * bcm_plp_aperta2_timesync_do_sync
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timesync_do_sync(bcm_plp_access_t phy_info) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_timesync_framesync_t framesync;
    plp_aperta2_phymod_timesync_framesync_mode_t original_mode;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_timesync_framesync_mode_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &framesync);
    ...
    rv = plp_aperta2_phymod_timesync_framesync_mode_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &framesync);
    if ( rv != BCM_PM_IF_SUCCESS ) {
        goto ERR;
    }
...
/*!
 * bcm_plp_aperta2_timestamp_univ_set
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timestamp_univ_set(bcm_plp_access_t phy_info, unsigned int flags,
    ...
    rv = plp_aperta2_phymod_timestamp_univ_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags, type, &phymod_ts_univ);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*!
 * bcm_plp_aperta2_timestamp_univ_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_timestamp_univ_get(bcm_plp_access_t phy_info, unsigned int flags,
                           int type, bcm_plp_timestamp_univ_t *timeuniv) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_timestamp_univ_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags, type, &phymod_ts_univ);
...
int bcm_timesync_ptp_action_set(bcm_plp_access_t phy_info, unsigned int flags,
    ...
    rv = plp_aperta2_phymod_timesync_ptp_action_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags,
...
int bcm_timesync_ptp_action_get(bcm_plp_access_t phy_info, unsigned int flags,
    ...
    rv = plp_aperta2_phymod_timesync_ptp_action_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, flags,
                                                       direction, mode, user_def);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
/*!
 * bcm_plp_aperta2_l1_intr_status_get
 *
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_l1_intr_status_get(bcm_plp_access_t phy_info, bcm_plp_l1_intr_status_t *l1_intr_status) {
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
...
#ifdef PHYMOD_EVORA_SUPPORT
    plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy.access.devad = 1;
    rv = plp_aperta2_phymod_phy_reg_read(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, 0x1008bfb, &l1_intr_status->l1_intr_sts);
#else
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_core_diagnostics_get(bcm_plp_access_t phy_info,  bcm_plp_core_diagnostics_t* core_diag)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    plp_aperta2_phymod_core_diagnostics_t phy_core_diag;

    PHYMOD_IF_MEMSET(&phy_core_diag, 0, sizeof(plp_aperta2_phymod_core_diagnostics_t));
    BCM_PM_MUTEX_LOCK(phy_info.phy_addr, phy_info.platform_ctxt);
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_CORE_ACCESS(phy_info, phy_id_idx);

    rv = plp_aperta2_phymod_core_diagnostics_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, &phy_core_diag);
...
/*!
 * bcm_plp_aperta2_avs_config_set
 *
 ...
 */
int bcm_plp_aperta2_avs_config_set(bcm_plp_access_t phy_info, bcm_plp_avs_config_t avs_config)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
...
#endif
    rv = plp_aperta2_phymod_phy_avs_config_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, phy_avs_config);
...
/*!
 * bcm_plp_aperta2_avs_config_get
 *
 * @brief AVS configuration
 *
 * @param phy_info <pre> Represents PHY access\n
 ...
 */
int bcm_plp_aperta2_avs_config_get(bcm_plp_access_t phy_info, bcm_plp_avs_config_t* avs_config)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_phy_avs_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phy_avs_config);
...
/*!
 * bcm_plp_aperta2_avs_status_get
 *
 * @brief AVS configuration
 *
 * @param phy_info <pre> Represents PHY access\n
 ...
 */
int bcm_plp_aperta2_avs_status_get(bcm_plp_access_t phy_info, bcm_plp_avs_config_status_t* avs_status)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_phy_avs_status_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phy_avs_status);
...
/*!
 * bcm_plp_aperta2_prbs_error_inject_set
 *
...
int bcm_plp_aperta2_prbs_error_inject_set(bcm_plp_access_t phy_info, bcm_plp_prbs_error_inject_t prbs_error_inject)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_phy_prbs_error_inject_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, error_inject);
...
/*!
 * bcm_plp_aperta2_logical_lane_set
 *
 ...
 */
int bcm_plp_aperta2_logical_lane_set(bcm_plp_access_t phy_info, bcm_plp_logical_lane_map_t logical_lane)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_logical_lane_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, &phy_logical_lane);
...
/*!
 * bcm_plp_aperta2_logical_lane_get
 *
 ...
 */
int bcm_plp_aperta2_logical_lane_get(bcm_plp_access_t phy_info, bcm_plp_logical_lane_map_t *logical_lane)
{
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    int rv = BCM_PM_IF_SUCCESS;
    ...
    rv = plp_aperta2_phymod_logical_lane_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, &phy_logical_lane);
 ...
 *	@return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_prbs_error_analyzer_proj(bcm_plp_access_t phy_info, unsigned short prbs_error_fec_size, unsigned char hist_errcnt_thresh, unsigned int timeout_s)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_prbs_error_analyzer_proj(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, prbs_error_fec_size, hist_errcnt_thresh, timeout_s);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
...
int bcm_plp_aperta2_dsrds_firmware_lane_config_set(bcm_plp_access_t phy_info, bcm_plp_dsrds_firmware_lane_config_t firmware_lane_config)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_dsrds_firmware_lane_config_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, fw_config);
...
int bcm_plp_aperta2_dsrds_firmware_lane_config_get(bcm_plp_access_t phy_info, bcm_plp_dsrds_firmware_lane_config_t* firmware_lane_config)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_dsrds_firmware_lane_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &fw_config);
...
int bcm_plp_aperta2_synce_config_set(bcm_plp_access_t phy_info, bcm_plp_synce_cfg_t* synce_cfg)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_synce_config_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phy_synce_cfg);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
    return rv;
}
...
int bcm_plp_aperta2_synce_config_get(bcm_plp_access_t phy_info, bcm_plp_synce_cfg_t* synce_cfg)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx;
    uint32_t exist_phy = 0;
...
#endif
    rv = plp_aperta2_phymod_synce_config_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phy_synce_cfg);
...
int bcm_get_hwport_number(bcm_plp_access_t phy_info, int *port_number)
{
    ...
    PHYMOD_IF_ERR_RETURN(_barchetta_get_package_info(&(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy)->access, &pkg_info));
    rv = _barchetta_retrieve_hardware_port_number_from_sw_database(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, pkg_info, port_number);
ERR:
    BCM_PM_MUTEX_UNLOCK(phy_info.phy_addr, phy_info.platform_ctxt);
 ...
 *  @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_pcs_status_get(bcm_plp_access_t phy_info, bcm_plp_pcs_status_t* pcs_status)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_phy_pcs_status_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &phymod_pcs_status);
...
/*!
 * bcm_plp_aperta2_pattern_enable_set
 *
 ...
 * @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_pattern_enable_set(bcm_plp_access_t phy_info, const bcm_plp_pattern_t *pattern)
{
    ...
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    plp_aperta2_phymod_pattern_t phymod_pattern;
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_PHY_ACCESS(phy_info, phy_id_idx);
    PHYMOD_MEMSET(&phymod_pattern, 0, sizeof(plp_aperta2_phymod_pattern_t));
    phymod_pattern.pattern_len = pattern->pattern_len;
    phymod_pattern.pattern = pattern->pattern;
    rv = plp_aperta2_phymod_phy_pattern_enable_set(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, pattern->enable, &phymod_pattern);
...
/*!
 * bcm_plp_aperta2_pattern_enable_get
 *
 ...
 * @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_pattern_enable_get(bcm_plp_access_t phy_info, bcm_plp_pattern_t *pattern)
{
    ...
    PHYMOD_MEMSET(&phymod_pattern, 0, sizeof(plp_aperta2_phymod_pattern_t));
    phymod_pattern.pattern = pattern->pattern;
    rv = plp_aperta2_phymod_phy_pattern_enable_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &pattern->enable, &phymod_pattern);
...
/*!
 * bcm_plp_aperta2_fw_init_params_get
 *
 * @brief Get the firmware init params configured during bcm_plp_aperta2_init_fw_bcast
 *
 * @param [in]  phy_info         - phy access information
 ...
 * @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_fw_init_params_get(bcm_plp_access_t phy_info, void* const fw_init_params)
{
    ...
    BCM_PLP_PHY_GET_P_CTXT(phy_info, phy_id_idx, exist_phy);
    BCM_PLP_FILL_CORE_ACCESS(phy_info, phy_id_idx);
    rv = plp_aperta2_phymod_phy_fw_init_params_get(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->core->pm_core, fw_init_params);
 ...
 * @return BCM_PM_IF_SUCCESS(0) for success and corresponding error code on failure
 */
int bcm_plp_aperta2_srds_diag_access_enable(bcm_plp_access_t phy_info, bcm_plp_srds_diag_access_cfg_t *diag_access_cfg) {
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = BCM_PM_IF_MAX_PHY;
    uint32_t exist_phy = 0;
    ...
    rv = plp_aperta2_phymod_srds_diag_access_enable(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, &srds_diag_access_cfg);
...
int bcm_plp_aperta2_phy_pai_info_get(bcm_plp_access_t phy_info, bcm_plp_pai_info_t *pai_info)
{
    int rv = BCM_PM_IF_SUCCESS;
    uint32_t phy_id_idx = 0;
    uint32_t exist_phy = 0;
    ...
    if (pai_info->pai_phy_config->config_type == PAI_PHY_INIT) {
...
#if defined(PHYMOD_PHY542XX_SUPPORT)
        rv = bcm_plp_aperta2_init(phy_info, pai_info->read, pai_info->write,
                  pai_init_info->firmware_load_type->firmware_load_method);
#else
        rv = bcm_plp_aperta2_init_fw_bcast(phy_info, pai_info->read, pai_info->write,
        ...
        PHYMOD_MEMSET(&bus, 0, sizeof(plp_aperta2_phymod_bus_t));

        if (pai_info->pai_phy_config->config_type == PAI_PHY_CONFIG_TYPE_FW_INIT) {
            rv = bcm_plp_aperta2_phy_get_phy_type (phy_info, &phy_type, pai_info->read,pai_info->write);
            if (rv != 0) {
                goto ERR;
            }
            ...
            PHYMOD_MEMCPY(&phy_access, &plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, sizeof(plp_aperta2_phymod_phy_access_t));
        }
        phymod_pai_phy_config.config_data = pai_info->pai_phy_config->config_data;

        rv = plp_aperta2_phymod_phy_pai_info_get(&phy_access, (phymod_pai_phy_op_t)pai_info->operation, 
...
int bcm_plp_aperta2_phy_stats(bcm_plp_access_t phy_info, bcm_plp_mib_stat_operation_t op_type, unsigned int *stat_id,
    unsigned int stats_count, plp_uint64_t *stats_p)
{
    ...
    rv = plp_aperta2_phymod_phy_stats(&plp_aperta2_phy_ctrl.phy[phy_id_idx]->pm_phy, op_type, stat_id, stats_count, stats_p);
