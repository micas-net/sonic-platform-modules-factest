 ...
 * -------------------------------------------------------------------------------- *
 * This sample application configures the Aperta2 chip for 800G(8x106G) PAM4 with   *
 ...
 *   4. Configure operating mode parameters for 800G(8x106G) PAM4 mode              *
 *   5. Run phydump                                                                 *
 *   6. Set/Get TxFir settings                                                      *
 *   7. Test Traffic                                                                *
...
int main(int argc, char *argv[])
{
    ...
    /* -------------------------------------------------------------------------------- *
     * Phy Status Dump on SYS and LINE side                                             *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        line_lane_map = line_lane_map_list[lane_map_index];
        ...
        for (side = SYS_SIDE; side >= LINE_SIDE; side--) {
            phy_info.flags    = BCM_PLP_INTERNAL_DUMP_L3;
    ...
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        ...
        for(side = BCM_SYSTEM_SIDE; side >= BCM_LINE_SIDE; side--) {
            memset(&tx_fir, 0, sizeof(bcm_plp_pam4_tx_t));
            phy_info.if_side  = (side == BCM_SYSTEM_SIDE) ? BCM_SYSTEM_SIDE : BCM_LINE_SIDE;
            phy_info.lane_map = (side == BCM_SYSTEM_SIDE) ? sys_lane_map : line_lane_map;
            /* First get FIR settings before updating */
            tx_fir.serdes_tx_tap_mode = TX_SET_PAM4_TAP_MODE;
            retval = bcm_plp_pam4_tx_get(CHIP_NAME, phy_info, &tx_fir);
            if (retval != TEST_SUCCESS) {
                printf("FAIL: PHY-%d: %s: lane_map=0x%2x: Failed to get TX parameters (ret = %d)\n",
                phy_info.phy_addr, (side == BCM_SYSTEM_SIDE) ? "SYS " : "LINE", phy_info.lane_map, retval);
                goto _aperta2_config_error;
            } else {
                printf("PASS: PHY-%d: %s: lane_map=0x%2x: TX parameters get successful\n",
                phy_info.phy_addr, (side == BCM_SYSTEM_SIDE) ? "SYS " : "LINE", phy_info.lane_map);
            }

            tx_fir.pre3  = TX_SET_PAM4_PRE3;
            tx_fir.pre2  = TX_SET_PAM4_PRE2;
            tx_fir.pre   = TX_SET_PAM4_PRE;
            tx_fir.main  = TX_SET_PAM4_MAIN;
            tx_fir.post  = TX_SET_PAM4_POST;
            tx_fir.post2 = TX_SET_PAM4_POST2;
            tx_fir.serdes_tx_tap_mode = TX_SET_PAM4_TAP_MODE;
            retval = bcm_plp_pam4_tx_set(CHIP_NAME, phy_info, &tx_fir);
            if (retval != TEST_SUCCESS) {
                printf("FAIL: PHY-%d: %s: lane_map=0x%2x: Failed to set TX parameters (ret = %d)\n",
                phy_info.phy_addr, (side == BCM_SYSTEM_SIDE) ? "SYS " : "LINE", phy_info.lane_map, retval);
                goto _aperta2_config_error;
            } else {
                printf("PASS: PHY-%d: %s: lane_map=0x%2x: TX parameters set successful\n",
                phy_info.phy_addr, (side == BCM_SYSTEM_SIDE) ? "SYS " : "LINE", phy_info.lane_map);
            }
        }
    }
