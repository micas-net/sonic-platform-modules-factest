 ...
 * -------------------------------------------------------------------------------- *
 * This sample application configures the Aperta2 chip for 50G(1x53G) PAM4 with     *
 ...
 *   4. Configure operating mode with System side OctalCrossing and system side     *
 *      Failover                                                                    *
 *   5. Setup link training on System side (primary and FOV both) and Line side     *
 *   6. Test Traffic                                                                *
 *   7. Failover Mode set to Seccondary port (Failover port)                        * 
 *   8. Test Traffic                                                                *
 *   9. Cleanup and close the connection to the board.                              *
 * -------------------------------------------------------------------------------- *
...
#define SYS_FEC_MODE            bcmplpaperta2RS544
...
#define SYS_OCTAL_CROSSING      bcmplpSysOctalCrossing
...
unsigned int failover_lane_map_list[] = {
...
#define LINE_FEC_MODE           bcmplpaperta2RS544
...
#define LINE_OCTAL_CROSSING     bcmplpSysOctalCrossing
...
int main(int argc, char *argv[])
{
    ...
    firmware_load_type.firmware_load_method = 0;
    firmware_load_type.force_load_method = 0;   
    ...
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        ...
        line_lane_map = line_lane_map_list[lane_map_index];
        failover_lane_map = failover_lane_map_list[lane_map_index];
        ...
        aux_mode_sys_set.failover_config.lane_map = failover_lane_map;
        ...
        aux_mode_line_set.failover_config.lane_map = 0x0;
    ...
    /* -------------------------------------------------------------------------------- *
     * Start Force Tx training at system side FOV port                                  *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        sys_lane_map = failover_lane_map_list[lane_map_index];
        phy_info.if_side = BCM_SYSTEM_SIDE;
        phy_info.lane_map = sys_lane_map ;
        retval = bcm_plp_force_tx_training_set(CHIP_NAME, phy_info, ENABLE);
        if (retval != TEST_SUCCESS) {
    ...
    sleep(5);
    /* -------------------------------------------------------------------------------- *
     * Get Force Tx training status at System side FOV port                             *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        sys_lane_map = failover_lane_map_list[lane_map_index];
    ...
    /* -------------------------------------------------------------------------------- *
     * Ports are setup now and ready to test with traffic on primary ports              *
     * -------------------------------------------------------------------------------- */
    printf("\n\n++++++++++++++++++++++++++++++++++++++++++++++++\n");
    printf("MacSec is in static bypass mode\n");
    printf("Ports are setup now and ready to test with traffic on primary ports\n");
    printf("++++++++++++++++++++++++++++++++++++++++++++++++\n");   

    /* -------------------------------------------------------------------------------- *
     * Section 6: Failover mode set                                                     *
     * -------------------------------------------------------------------------------- */
    phy_info.phy_addr = PHY_ID;
    for (lane_map_index = 0; lane_map_index < total_lane_maps; lane_map_index++) {
        sys_lane_map = sys_lane_map_list[lane_map_index];
        phy_info.if_side = BCM_SYSTEM_SIDE;
        phy_info.lane_map = sys_lane_map ;
        retval = bcm_plp_failover_mode_set(CHIP_NAME, phy_info, FAILOVER_ENABLE);
        if (retval != TEST_SUCCESS) {
            printf("FAIL: Failed to set failover_mode for lane-map 0x%x of PHY-%d at System side (ret = %d)!\n",
            phy_info.lane_map, phy_info.phy_addr, retval);
            goto _aperta2_config_error;
        } else {
            printf("failover_mode set done for lane map 0x%x of PHY-%d at System side...\n",
            phy_info.lane_map, phy_info.phy_addr);
        }
    }
    ...
    /* -------------------------------------------------------------------------------- *
     * Ports are setup now and ready to test with traffic on fov ports                             *
     * -------------------------------------------------------------------------------- */
     printf("\n\n++++++++++++++++++++++++++++++++++++++++++++++++\n");
     printf("MacSec is in static bypass mode\n");
     printf("Ports are setup now and ready to test with traffic on fov ports\n");
     printf("++++++++++++++++++++++++++++++++++++++++++++++++\n");
