...
static int fpga_device_write(fpga_i2c_dev_t *fpga_i2c, uint32_t pos, uint8_t *val, size_t size)
{
    ...
    pfunc = (device_func_write)fpga_i2c->write_intf_addr;
    return pfunc(fpga_i2c->dev_name, pos, val, size);
}
...
static int fpga_device_read(fpga_i2c_dev_t *fpga_i2c, uint32_t pos, uint8_t *val, size_t size)
{
    ...
    pfunc = (device_func_read)fpga_i2c->read_intf_addr;
    return pfunc(fpga_i2c->dev_name, pos, val, size);
}
...
static int fpga_i2c_config_init(fpga_i2c_dev_t *fpga_i2c)
{
    ...
    if (dev->of_node) {
        ...
        ret = find_intf_addr(&fpga_i2c->write_intf_addr, &fpga_i2c->read_intf_addr, fpga_i2c->i2c_func_mode);
        if (ret) {
            dev_err(dev, "find_intf_addr func mode %d fail, ret: %d.\n", fpga_i2c->i2c_func_mode, ret);
            return ret;
        }

        if (!fpga_i2c->write_intf_addr || !fpga_i2c->read_intf_addr) {
        ...
        if (rv == 0) {
            ...
            DEBUG_VERBOSE("fpga i2c data buf length reg addr: 0x%x, value: %d\n",
        ...
        if (rv == 0) {
            ...
            DEBUG_VERBOSE("fpga i2c adapter offset reg addr: 0x%x, value: %d\n",
    ...
    DEBUG_VERBOSE("i2c_ext_9548_exits_flag:0x%x, i2c_in_9548_chan:0x%x, i2c_data_buf:0x%x, i2c_reg_len:0x%x, i2c_data_buf_len:0x%x.\n",
...
static int fpga_i2c_probe(struct platform_device *pdev)
{
    ...
    DEBUG_VERBOSE("fpga i2c params check flag:%d.\n", fpga_i2c->i2c_params_check);
