...
/*********************start: reg addr *******************/
/* LCR DLAB=1 */
#define UART_DLL    (0x00)    /* R/W: Divisor Latch Low */
#define UART_DLM    (0x04)    /* R/W: Divisor Latch High */
#define UART_FCR_RO (0x08)    /* RO: FIFO Control Register */

/* LCR DLAB=0 */
#define UART_RX     (0x00)    /* RO:  Receive buffer */
#define UART_TX     (0x00)    /* WO: Transmit buffer */
#define UART_IER    (0x04)    /* R/W: Interrupt Enable Register */
#define UART_IIR    (0x08)    /* RO:  Interrupt ID Register */
#define UART_FCR_WO (0x08)    /* WO: FIFO Control Register */

/* LCR DLAB=x */
#define UART_LCR    (0x0c)    /* R/W: Line Control Register */
#define UART_LSR    (0x14)    /* RO:  Line Status Register */

/* FPGA only have RX\TX, Modem reg not use */
#define UART_MCR    (0x10)    /* R/W: Modem Control Register */
#define UART_MSR    (0x18)    /* RO:  Modem Status Register */
#define UART_SCR    (0x1c)    /* R/W: Scratch Register */
...
#define UART_IER_THRI           (0x02) /* Enable Transmitter holding register empty int. */
...
#define UART_FCR_DMA_SELEC      (0x08) /* For DMA applications */
#define UART_FCR_TRIGGER_MAS    (0xC0) /* Mask for the FIFO trigger range */
...
#define UART_CLK_SOURCE_FREQ_DEF   (62500000)
#define UART_MAX_RX_FIFO_SIZE      (1024)
#define UART_TX_FIFO_SIZE_DEF      (1)

/* All serial ports shared registers */
#define UART_REG_SPD_CFG_VAL   (0x400)
#define UART_REG_SPD_DETECT    (0x404)
#define UART_REG_SPD_RELIABLE  (0x408)
#define UART_REG_IMPLICIT0     (0x40c)
#define UART_REG_IMPLICIT1     (0x410)
#define UART_REG_IMPLICIT2     (0x414)
...
static u32 uart_probe_baud(struct uart_port *port)
{
    ...
    port->serial_out(port, UART_LCR, lcr | UART_LCR_DLAB);
    dll = port->serial_in(port, UART_DLL);
    dlm = port->serial_in(port, UART_DLM);
...
static void uart_get_detected_baud(struct uart_port *port, u32 *pbaud)
{
    ...
    offset = UART_REG_SPD_DETECT - ((unsigned long)port->membase & (~PAGE_MASK));
...
static void uart_hw_init(struct uart_port *port)
{
    ...
    fcr_reg_val = UART_FCR_ENABLE_FIFO;
    port->serial_out(port, UART_FCR_WO, fcr_reg_val);
    DEBUG_INFO("w UART_FCR, addr: 0x%x, val:  0x%x\n", UART_FCR_WO, fcr_reg_val);

    /* 1.1 set FCR: clear RX TX FIFO */
    fcr_reg_val = UART_FCR_ENABLE_FIFO | UART_FCR_CLEAR_RCVR | UART_FCR_CLEAR_XMIT;
    port->serial_out(port, UART_FCR_WO, fcr_reg_val);
    DEBUG_INFO("w UART_FCR, addr: 0x%x, val:  0x%x\n", UART_FCR_WO, fcr_reg_val);

    /* 1.2 set FCR: enable FIFO, and set RXFIFO trigger level */
    fcr_reg_val = UART_FCR_ENABLE_FIFO | UART_FCR_TRIGGER_14;
    port->serial_out(port, UART_FCR_WO, fcr_reg_val);
    DEBUG_INFO("w UART_FCR, addr: 0x%x, val:  0x%x\n", UART_FCR_WO, fcr_reg_val);

    /* 2. set LCR: date 8bit, 1bit stop, no parity, close break condition, DLAB = 0 */
    lcr_reg_val = UART_LCR_WLEN8;
    port->serial_out(port, UART_LCR, lcr_reg_val);
    DEBUG_INFO("w UART_LCR, addr: 0x%x, val:  0x%x\n", UART_LCR, lcr_reg_val);
    ...
    /* 3. clear status reg */
    tmp1 = port->serial_in(port, UART_LSR);
    (void)port->serial_in(port, UART_RX);
    tmp2 = port->serial_in(port, UART_IIR);
    (void)port->serial_in(port, UART_MSR);

    DEBUG_INFO("r UART_LSR, addr: 0x%x, val: 0x%x, tmp1: 0x%x\n", \
               UART_LSR, port->serial_in(port, UART_LSR), tmp1);
    DEBUG_INFO("r UART_IIR, addr: 0x%x, val: 0x%x, tmp2: 0x%x\n", \
               UART_IIR, port->serial_in(port, UART_IIR), tmp2);

    /* 4. set IER: enable irq */
    ier_reg_val = UART_IER_RDI | UART_IER_RLSI;
    port->serial_out(port, UART_IER, ier_reg_val);
    DEBUG_INFO("w UART_IER, addr: 0x%x, val: 0x%x\n", UART_IER, ier_reg_val);
...
static void uart_hw_deinit(struct uart_port *port)
{
    ...
    /* 2. close FIFO */
    port->serial_out(port, UART_FCR_WO, 0);
...
static int uart_rx_one_char(struct uart_port *port)
{
    ...
    if ((lsr & (UART_LSR_DR | UART_LSR_BRK_ERROR_BITS)) == 0) {
...
static void uart_rx_chars(struct uart_port *port)
{
    ...
    rx_fifo_size = UART_MAX_RX_FIFO_SIZE;
...
static void uart_tx_chars(struct uart_port *port)
{
    ...
    if (port->x_char) {
        ...
        port->serial_out(port, UART_TX, port->x_char);
        port->icount.tx++;
        port->x_char = 0;
        return;
    }
    ...
    for (i = 0; i < port->fifosize; i++) {
        ...
        port->serial_out(port, UART_TX, xmit->buf[xmit->tail]);
...
static unsigned int wb_uart_tx_empty(struct uart_port *port)
{
    ...
    spin_lock_irqsave(&port->lock, flags);
    stat = port->serial_in(port, UART_LSR);
    spin_unlock_irqrestore(&port->lock, flags);

    DEBUG_INFO("stat: 0x%x\n", stat);

    return (stat & BOTH_EMPTY) == BOTH_EMPTY ? TIOCSER_TEMT : 0;
}
...
static void uart_cfg_lcr(struct uart_port *port, tcflag_t c_cflag)
{
    ...
    port->serial_out(port, UART_LCR, cval);
    DEBUG_INFO("w UART_LCR, addr: 0x%x, val:  0x%x\n", UART_LCR, cval);
...
static int wb_uart_probe(struct platform_device *pdev)
{
    ...
    p_uart_ocore->port.fifosize = tx_fifo_size ? tx_fifo_size : UART_TX_FIFO_SIZE_DEF;
    p_uart_ocore->port.uartclk  = clk_freq ? clk_freq : UART_CLK_SOURCE_FREQ_DEF;
