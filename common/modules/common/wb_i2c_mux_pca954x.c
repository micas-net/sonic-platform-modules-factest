...
static int pca9548_gpio_init(gpio_attr_t *gpio_attr)
{
    ...
    if (gpio_attr->gpio_init) {
        DEBUG_VERBOSE("gpio%d already init, do nothing.\n", gpio_attr->gpio);
        return 0;
    }

    DEBUG_VERBOSE("gpio%d init.\n", gpio_attr->gpio);
    ...
    return 0;
error:
    DEBUG_ERROR("pca9548_gpio_init failed, ret:%d.\n", err);
    return err;
}
            ...
            unsigned char *buf, uint32_t size)
{
    ...
    for (j = 0 ;j < size ;j++) {
        for (i = 0; i < I2C_RETRY_TIMES; i++) {
            ...
            if (rv < 0) {
                DEBUG_ERROR("i2c read failed, try again.\n");
            ...
            uint8_t write_buf)
{
    ...
    for (i = 0; i < I2C_RETRY_TIMES; i++) {
        ...
        if (rv < 0) {
            DEBUG_ERROR("i2c write failed, try again.\n");
            msleep(I2C_RETRY_WAIT_TIMES);
            if (i >= (I2C_RETRY_TIMES - 1)) {
                goto out;
            }
...
static int pca954x_do_file_reset(struct i2c_mux_core *muxc)
{
    ...
    DEBUG_VERBOSE("rst_delay_b:%u, rst_delay:%u, rst_delay_a:%u.\n",
        reset_cfg->rst_delay_b, reset_cfg->rst_delay, reset_cfg->rst_delay_a);
    DEBUG_VERBOSE("dev_name:%s, offset:0x%x, mask:0x%x, on:0x%x, off:0x%x, width: %u\n",
    ...
    if (file_attr->width == WIDTH_1Byte) {
        ...
        DEBUG_VERBOSE("1 byte write reset on val[0]: 0x%x, write reset off valu[0]: 0x%x\n",
        ...
        DEBUG_VERBOSE("4 byte write reset on val[0]:0x%x, val[1]:0x%x, val[2]:0x%x, val[3]:0x%x",
            write_reset_on_value[0], write_reset_on_value[1], write_reset_on_value[2], write_reset_on_value[3]);
        DEBUG_VERBOSE("4 byte write reset off val[0]:0x%x, val[1]:0x%x, val[2]:0x%x, val[3]:0x%x",
    ...
    while (timeout > 0) {
        ...
        if (tmp_read32 == file_attr->reset_off) {
            ret = 0;
            DEBUG_VERBOSE("pca954x_do_file_reset success.\n");
            break;
        }
        udelay_cnt++;
    ...
    if (ret < 0) {
        DEBUG_ERROR("pca954x_do_file_reset timeout.\n");
    }
out:
    if (err < 0) {
        DEBUG_ERROR("pca954x_do_file_reset file rd/wr failed, ret:%d.\n", err);
    }
...
static int pca954x_do_io_reset(struct i2c_mux_core *muxc)
{
    ...
    DEBUG_VERBOSE("rst_delay_b:%u, rst_delay:%u, rst_delay_a:%u.\n",
        reset_cfg->rst_delay_b, reset_cfg->rst_delay, reset_cfg->rst_delay_a);
    DEBUG_VERBOSE("io_addr:0x%x, mask:0x%x, on:0x%x, off:0x%x.\n",
    ...
    while (timeout > 0) {
        ...
        if (val == io_attr->reset_off) {
            ret = 0;
            DEBUG_VERBOSE("pca954x_do_io_reset success.\n");
            break;
        }
        udelay_cnt++;
        if ((udelay_cnt % 1000) == 0) {
    ...
    if (ret < 0) {
        DEBUG_ERROR("pca954x_do_io_reset timeout.\n");
    }
...
static int pca954x_do_gpio_reset(struct i2c_mux_core *muxc)
{
    ...
    while (timeout > 0) {
        ...
        if (val == gpio_attr->reset_off) {
            ret = 0;
            DEBUG_VERBOSE("pca954x_do_gpio_reset success.\n");
            break;
        }
        udelay_cnt++;
        if ((udelay_cnt % 1000) == 0) {
    ...
    if (ret < 0) {
        DEBUG_ERROR("pca954x_do_gpio_reset timeout.\n");
    }
...
static int pca954x_do_i2c_reset(struct i2c_mux_core *muxc)
{
    ...
    DEBUG_VERBOSE("rst_delay_b:%u, rst_delay:%u, rst_delay_a:%u.\n",
        reset_cfg->rst_delay_b, reset_cfg->rst_delay, reset_cfg->rst_delay_a);
    DEBUG_VERBOSE("bus:0x%x, addr:0x%x, reg:0x%x, mask:0x%x, on:0x%x, off:0x%x.\n",
    ...
    while (timeout > 0) {
        ...
        if (val == i2c_attr->reset_off) {
            ret = 0;
            DEBUG_VERBOSE("pca954x_do_i2c_reset success.\n");
            break;
        }
        udelay_cnt++;
        if ((udelay_cnt % 1000) == 0) {
    ...
    if (ret < 0) {
        DEBUG_ERROR("pca954x_do_i2c_reset timeout.\n");
    }
out:
    if (err < 0) {
        DEBUG_ERROR("pca954x_do_i2c_reset i2c op failed, ret:%d.\n", err);
    }
    return ret;
}
...
static int pca954x_do_reset(struct i2c_mux_core *muxc)
{
    ...
    if (data->pca9548_cfg_info.pca9548_reset_type == PCA9548_RESET_NONE) {
        ret = -1;
        DEBUG_VERBOSE("Don't need to reset.\n");
        ...
        ret = -1;
        DEBUG_ERROR("Unsupport reset type:0x%x.\n",
            data->pca9548_cfg_info.pca9548_reset_type);
    }

    if (ret < 0) {
        DEBUG_ERROR("pca9548_reset_ctrl failed, reset type:%u, ret:%d.\n",
...
static int pca954x_select_chan(struct i2c_mux_core *muxc, u32 chan)
{
    ...
    if (data->pca9548_cfg_info.select_chan_check) { /* check chan */
        ...
        if ((ret < 0) || (read_val != data->last_chan)) {
            ...
            DEBUG_VERBOSE("ret = %d, read_val = %d, last_chan = %d.\n", ret, read_val, data->last_chan);
            rv = pca954x_do_reset(muxc);
            if (rv >= 0) {
                DEBUG_VERBOSE("pca954x_do_reset success, rv = %d.\n", rv);
            } else {
                DEBUG_VERBOSE("pca954x_do_reset failed, rv = %d.\n", rv);
            }
            if (ret >= 0) {
...
static int pca954x_deselect_mux(struct i2c_mux_core *muxc, u32 chan)
{
    ...
    if (rv == 0) {
        DEBUG_VERBOSE("match 9641, close 9548 channel to deselect 9641.\n");
        (void)pca954x_reg_write(muxc->parent, client, data->last_chan);
    } else {
        DEBUG_VERBOSE("dismatch 9641, do nothing.\n");
    }
...
static int of_pca954x_reset_data_init(struct pca954x *data)
{
    ...
    if (dev == NULL || dev->of_node == NULL) {
        DEBUG_VERBOSE("dev or dev->of_node is NUll, no reset.\n");
    ...
    DEBUG_VERBOSE("reset_type:0x%x, rst_delay_b:0x%x, rst_delay:0x%x, rst_delay_a:0x%x.\n",
    ...
    if (reset_cfg->pca9548_reset_type == PCA9548_RESET_I2C) {
        ...
        DEBUG_VERBOSE("bus:%u, addr:0x%x, offset:0x%x, mask:0x%x, on:0x%x, off:0x%x.\n",
        ...
        DEBUG_VERBOSE("reset by gpio.\n");
        err = of_property_read_u32(dev->of_node, "gpio_num", &reset_cfg->attr.gpio_attr.gpio);
        ...
        DEBUG_VERBOSE("gpio number:%u, reset_on:0x%x, reset_off:0x%x.\n",
        ...
        DEBUG_VERBOSE("io_addr:0x%x, mask:0x%x, reset_on:0x%x, reset_off:0x%x.\n",
        ...
        if (rv == 0) {
            if ((reset_cfg->attr.file_attr.width != WIDTH_1Byte) && (reset_cfg->attr.file_attr.width != WIDTH_4Byte)) {
                DEBUG_ERROR("9548 reset config error, witdh: %u not support.\n", reset_cfg->attr.file_attr.width);
                return -EINVAL;
            }
        } else {
            reset_cfg->attr.file_attr.width = 1;
        }
        DEBUG_VERBOSE("dev_name:%s, mask:0x%x, reset_on:0x%x, reset_off:0x%x, width: %u\n",
        ...
        DEBUG_ERROR("Unsupport reset type:%d.\n", reset_cfg->pca9548_reset_type);
        goto dts_config_err;
    }
    return 0;
dts_config_err:
...
static int pca954x_reset_data_init(struct pca954x *data)
{
    ...
    if (data->client->dev.platform_data == NULL) {
        DEBUG_VERBOSE("pca954x has no reset platform data config.\n");
        return 0;
    }
    reset_cfg = &data->pca9548_cfg_info;
             ...
             const struct i2c_device_id *id)
{
    ...
    unsigned int probe_disable, probe_hw_init;
    int ret, dynamic_nr;
    i2c_mux_pca954x_device_t *i2c_mux_pca954x_device;

    DEBUG_VERBOSE("pca954x_probe, parent bus: %d, 9548 addr:0x%x.\n", adap->nr, client->addr);
    ...
    if (client->dev.of_node == NULL) {
        if (client->dev.platform_data == NULL) {
            probe_disable = 1;
            DEBUG_VERBOSE("has no platform data config, set probe_disable = 1.\n");
        } else {
            i2c_mux_pca954x_device = client->dev.platform_data;
            probe_disable = i2c_mux_pca954x_device->probe_disable;
            probe_hw_init = i2c_mux_pca954x_device->probe_hw_init;
        }
    } else {
        probe_disable = of_property_read_bool(of_node, "probe_disable");
        probe_hw_init = of_property_read_bool(of_node, "probe_hw_init");
    }
    ...
    if(probe_hw_init) {
        if(i2c_smbus_write_byte(client, 0) < 0) {
            if(probe_disable) {
                DEBUG_VERBOSE("probe failed, continue go for creaate i2c adapater\n");
            } else {
                dev_info(&client->dev, "probe failed, exit\n");
                return -ENODEV;
            }
        }
    } else {
        DEBUG_VERBOSE("No need to probe_hw_init\n");
    }
    ...
    if (client->dev.of_node == NULL) {
        ...
        if (of_property_read_u32(of_node, "pca9548_base_nr", &data->pca9548_cfg_info.pca9548_base_nr)) {
            ...
            dynamic_nr = 1;
            DEBUG_VERBOSE("pca9548_base_nr not found, use dynamic adap number");
        } else {
            dynamic_nr = 0;
            DEBUG_VERBOSE("pca9548_base_nr:%u.\n", data->pca9548_cfg_info.pca9548_base_nr);
        }
    ...
    if(probe_hw_init) {
        ret = pca954x_do_reset(muxc);
        if (ret >= 0) {
            DEBUG_VERBOSE("pca954x_do_reset success, ret = %d.\n", ret);
        } else {
            DEBUG_VERBOSE("pca954x_do_reset failed, ret = %d.\n", ret);
        }
    }
