...
#define YT8521_RX_LPI_ACTIVE            9

#define YT8531S_LED0_CFG                0xa00c
#define YT8531S_LED1_CFG                0xa00d
#define YT8531S_LED2_CFG                0xa00e

#define YT_SW_RESET_TIMEOUT_TIME       (10000)
...
/* 函数功能：根据DTS来配置PHY YT8531S 预加重寄存器的值。
 * 输入参数：phydev --- phy设备指针
 * 输出参数：无
 * 返 回 值：无
 */
static void yt8531s_pre_emphasis_init(struct phy_device *phydev)
{
    int ret;
    ...
    /* 1. 查找设备节点是否存在 */
    np = of_find_node_by_name(NULL, "phy_yt8531s_pre_emphasis_cfg");
    if (np == NULL) {
        printk(KERN_INFO "yt8531s phy not cfg pre emphasis\n");
        return;
    }
    ...
    reg_addr = 0;
    reg_val  = 0;
    ret  = of_property_read_u32(np, "pre_emphasis_reg_addr", &reg_addr);
    ret |= of_property_read_u32(np, "pre_emphasis_reg_val",  &reg_val);
    if (ret != 0) {
        printk(KERN_ERR "get reg_addr or reg_addr property fail\n");
        return;
    }

    /* 3. 根据属性信息，对预加重寄存器进行配置 */
    /* 3.1 保存当前寄存器值 */
    saved_reg_val = ytphy_read_ext(phydev, 0xa000);
    if (saved_reg_val < 0) {
        printk(KERN_ERR "read reg fail, addr: 0xa000, val: 0x%x\n", saved_reg_val);
        return;
    }
    ...
    if (ret < 0) {
        printk(KERN_ERR "write ext reg fail, addr: 0xa000, val: 2, ret: %d\n", ret);
        return;
    }

    /* 3.3 预加重寄存器配置 */
    ret = ytphy_write_ext(phydev, reg_addr, (u16)reg_val);
    if (ret < 0) {
        printk(KERN_ERR "write ext reg fail, addr: 0x%x, val: 0x%x, ret: %d\n", 
                                    reg_addr, reg_val, ret);
    }

    /* 3.4 恢复保存的配置 */
    ret = ytphy_write_ext(phydev, 0xa000, (u16)saved_reg_val);
    if (ret < 0) {
        printk(KERN_ERR "resume reg fail, addr: 0xa000, val: 0x%x, ret: %d\n", 
                                    saved_reg_val, ret);
        return;
    }
...
static int yt8521_read_status(struct phy_device *phydev)
{
	...
	if (YT8521_PHY_MODE_CURR != YT8521_PHY_MODE_UTP) {
        ...
        /* phy serdes硬件缺陷规避： 如果进入LPI模式，软复位一下 */
        if ((val & BIT(YT8521_RX_LPI_ACTIVE)) != 0) {
            printk(KERN_INFO "sds rx into lpi mode, do sw reset to resume, val: 0x%x\n",val);
        
            /* 1. 配置软复位 */
            reg_val = phy_read(phydev, MII_BMCR);
            reg_val |= YT_SOFTWARE_RESET;
            (void)phy_write(phydev, MII_BMCR, reg_val);

            /* 2. 等待复位标记自清 */
            for (timeout = 0; timeout < YT_SW_RESET_TIMEOUT_TIME; timeout++) {
                udelay(10);
                reg_val = phy_read(phydev, MII_BMCR);
                if ((reg_val & YT_SOFTWARE_RESET) == 0) {
                    break;
                }
            }

            if (timeout >= YT_SW_RESET_TIMEOUT_TIME) {
                printk(KERN_INFO "phy sds do sw reset timeout\n");
            } else {
                printk(KERN_INFO "phy sds do sw reset ok\n");
            }
        }
...
/* 函数功能：根据DTS来配置YT8531S PHY LED寄存器的值。
 * 输入参数：phydev    phy设备指针
 * 输出参数：无
 * 返 回 值：led配置失败与否，不影响mgmt的实际通信，故无返回值。
 */
static void yt8531s_led_init(struct phy_device *phydev)
{
    int ret;
    struct device_node *np;
    u32 led0_cfg_val, led1_cfg_val, led2_cfg_val;

    np = of_find_node_by_name(NULL, "yt8531s-phy-led");
    if (np == NULL) {
        printk(KERN_INFO "no find device node: yt8531s-phy-led\n");
        return;
    }
    ...
    led1_cfg_val = 0;
    led2_cfg_val = 0;
    ret = of_property_read_u32(np, "led0_cfg", &led0_cfg_val);
    ret |= of_property_read_u32(np, "led1_cfg", &led1_cfg_val);
    ret |= of_property_read_u32(np, "led2_cfg", &led2_cfg_val);

    if (ret != 0) {
        printk(KERN_INFO "get led0_cfg or led1_cfg or led2_cfg property fail\n");
        return;
    }

    ret = ytphy_write_ext(phydev, YT8531S_LED0_CFG, led0_cfg_val);
    ret |= ytphy_write_ext(phydev, YT8531S_LED1_CFG, led1_cfg_val);
    ret |= ytphy_write_ext(phydev, YT8531S_LED2_CFG, led2_cfg_val);
    if (ret < 0) {
        printk(KERN_INFO "write led cfg reg fail\n");
        return;
    }
...
static int yt8531S_config_init(struct phy_device *phydev)
{
    ...
    yt8531s_pre_emphasis_init(phydev);
