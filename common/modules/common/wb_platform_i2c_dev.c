...
static struct platform_i2c_dev_info* i2c_dev_arry[MAX_DEV_NUM];

typedef struct platform_i2c_dev_info {
    uint32_t i2c_bus;
    uint32_t i2c_addr;
    const char *name;
    const char *alias;
...
static int transfer_read(struct platform_i2c_dev_info *i2c_dev, u8 *buf, loff_t regaddr, size_t count)
{
    ...
    if (adap == NULL) {
        DEBUG_ERROR("get i2c adapter %d faild.\n", i2c_dev->i2c_bus);
        return -ENXIO;
    }
            ...
            if (i2c_check_functionality(adap, I2C_FUNC_SMBUS_WRITE_I2C_BLOCK)) {
                for (j = 0; j < count; j += I2C_SMBUS_BLOCK_MAX) {
                    ...
                    ret = adap->algo->smbus_xfer(adap, i2c_dev->i2c_addr,
                                    ...
                                    offset, I2C_SMBUS_I2C_BLOCK_DATA, &data);
                    if (ret) {
                        DEBUG_ERROR("smbus_xfer write block error, ret = %d\n", ret);
                        ret = -EFAULT;
                        goto error_exit;
                    }
                    offset += length;
                ...
                for (j = 0; j < count; j++) {
                    ...
                    ret = adap->algo->smbus_xfer(adap, i2c_dev->i2c_addr,
                                    ...
                                    offset, I2C_SMBUS_BYTE_DATA, &data);
                    if (ret) {
                        DEBUG_ERROR("smbus_xfer write byte error, ret = %d\n", ret);
                        ret = -EFAULT;
                        goto error_exit;
                    }
                    offset += 1;
...
static int device_read(struct platform_i2c_dev_info *i2c_dev, uint32_t offset, uint8_t *buf, size_t count)
{
    int i, j, ret;
    u8 tmp_offset;
    u8 val[MAX_RW_LEN];
...
static struct kobj_type platform_i2c_dev_ktype = {
...
{
    wb_platform_i2c_dev_t *i2c_dev = container_of(kobj, wb_platform_i2c_dev_t, kobj);
...
static int rw_status_check_one_time(wb_platform_i2c_dev_t *i2c_dev, uint32_t offset, uint32_t len, uint32_t type)
{
...
static int rw_status_check(wb_platform_i2c_dev_t *i2c_dev, uint32_t offset, uint32_t len, uint32_t type)
{
    int ret, i;
...
{
    wb_platform_i2c_dev_t *i2c_dev;
    uint32_t offset, type, len;
    int ret, i;

    i2c_dev = container_of(kobj, wb_platform_i2c_dev_t, kobj);
    if (!i2c_dev) {
...
static int of_platform_i2c_dev_config_init(struct platform_device *pdev, struct platform_i2c_dev_info *i2c_dev)
{
    int i, ret;
    uint32_t type;
    ...
    ret += of_property_read_u32(pdev->dev.of_node, "i2c_len", &i2c_dev->i2c_len);
    if (ret != 0) {
    ...
    if (i2c_dev->addr_bus_width == 0) {
        dev_err(&pdev->dev, "Invalid addr_bus_width: %u\n", i2c_dev->addr_bus_width);
        return -EINVAL;
    }

    if (of_property_read_string(pdev->dev.of_node, "i2c_alias", &i2c_dev->alias)) {
        i2c_dev->alias = i2c_dev->name;
    }

    ret = of_property_read_u32(pdev->dev.of_node, "status_check_type", &i2c_dev->status_check_type);
    if (ret == 0) {
        type = i2c_dev->status_check_type;
        ...
        ret = of_property_read_u32(pdev->dev.of_node, "test_reg_num", &i2c_dev->test_reg_num);
        if (ret != 0) {
        ...
        ret = of_property_read_u32_array(pdev->dev.of_node, "test_reg", i2c_dev->test_reg, i2c_dev->test_reg_num);
        if(ret != 0) {
    ...
    ret = of_property_read_u32(pdev->dev.of_node, "log_num", &(i2c_dev->log_node.log_num));
    if (ret == 0) {
        ...
        ret = of_property_read_u32_array(pdev->dev.of_node, "log_index", i2c_dev->log_node.log_index, i2c_dev->log_node.log_num);
        if (ret != 0) {
...
static int platform_i2c_dev_config_init(struct platform_device *pdev, struct platform_i2c_dev_info *i2c_dev)
{
    int i;
    uint32_t type;
    ...
    i2c_dev->i2c_len = platform_i2c_dev_device->i2c_len;
    if (i2c_dev->data_bus_width == 0) {
        dev_err(&pdev->dev, "Invalid data_bus_width: %u\n", i2c_dev->data_bus_width);
        return -EINVAL;
    }
    ...
    if (strlen(platform_i2c_dev_device->i2c_alias) == 0) {
        i2c_dev->alias = platform_i2c_dev_device->i2c_name;
    } else {
        i2c_dev->alias = platform_i2c_dev_device->i2c_alias;
    }

    i2c_dev->status_check_type = platform_i2c_dev_device->status_check_type;
    if (i2c_dev->status_check_type != NOT_SUPPORT_CHECK_STATUS) {
        ...
        i2c_dev->test_reg_num = platform_i2c_dev_device->test_reg_num;
        if ((i2c_dev->test_reg_num == 0) || (i2c_dev->test_reg_num > TEST_REG_MAX_NUM)) {
            dev_err(&pdev->dev, "Invalid test_reg_num: %u\n", i2c_dev->test_reg_num);
            return -EINVAL;
        }
        for (i = 0; i < i2c_dev->test_reg_num; i++) {
            i2c_dev->test_reg[i] = platform_i2c_dev_device->test_reg[i];
            DEBUG_VERBOSE("test_reg[%d] address = 0x%x.\n", i, i2c_dev->test_reg[i]);
        }
    ...
    i2c_dev->log_node.log_num = platform_i2c_dev_device->log_num;
    if (i2c_dev->log_node.log_num != 0) {
        ...
        for (i = 0; i < i2c_dev->log_node.log_num; i++) {
            i2c_dev->log_node.log_index[i] = platform_i2c_dev_device->log_index[i];
            DEBUG_VERBOSE("log_index[%d] address = 0x%x\n", i, i2c_dev->log_node.log_index[i]);
        }
    } else {
        DEBUG_VERBOSE("Don't support bsp key record.\n");
...
static int platform_i2c_dev_probe(struct platform_device *pdev)
{
    ...
    if (pdev->dev.of_node) {
        ret = of_platform_i2c_dev_config_init(pdev, i2c_dev);
    } else {
        ret = platform_i2c_dev_config_init(pdev, i2c_dev);
    }
    ...
    if ((i2c_dev->i2c_len == 0) || (i2c_dev->i2c_len & (i2c_dev->data_bus_width - 1))) {
        dev_err(&pdev->dev, "Invalid config i2c_len %d, data bus_width %d.\n",
    ...
    ret = kobject_init_and_add(&i2c_dev->kobj, &platform_i2c_dev_ktype, logic_dev_kobj, "%s", i2c_dev->name);
    if (ret) {
    ...
    dev_info(&pdev->dev, "register %u addr_bus_width %u data_bus_width 0x%x i2c_len device %s minor %d with %u per_rd_len %u per_wr_len success.\n",
...
static int platform_i2c_dev_remove(struct platform_device *pdev)
{
    ...
    if (i2c_dev->sysfs_group) {
        dev_dbg(&pdev->dev, "Unregister %s i2c_dev sysfs group\n", i2c_dev->name);
        sysfs_remove_group(&i2c_dev->kobj, (const struct attribute_group *)i2c_dev->sysfs_group);
        kobject_put(&i2c_dev->kobj);
    }

    dev_info(&pdev->dev, "Remove %s i2c device success.\n", i2c_dev->name);
