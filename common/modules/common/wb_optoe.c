...
#define OPTOE_CACHE_FILE_PATH  "/tmp/.optoe_%d_0x%x_cache"
#define OPTOE_MASK_FILE_PATH  "/tmp/.optoe_%d_0x%x_mask"
		...
		char *buf, loff_t off, size_t len, int opcode)
{
    ...
    if (optoe->file_cache_rd && opcode == OPTOE_READ_OP) {
        ret = cache_value_read(optoe->mask_file_path, optoe->cache_file_path, off, tmp_buf, len);
        if (ret < 0) {
            dev_err(&client->dev, "optoe data offset: 0x%llx, read_len: %lu, read cache file fail, ret: %d, return act value\n",
                off, len, ret);
        } else {
            dev_dbg(&client->dev, "optoe data offset: 0x%llx, read_len: %lu success, read from cache value\n",
                off, len);
        }
    }
...
static ssize_t mask_file_path_show(struct device *dev, struct device_attribute *dattr, char *buf) {
    ...
    memset(buf, 0, PAGE_SIZE);
    return snprintf(buf, PAGE_SIZE, "%s\n", optoe->mask_file_path);
}
static DEVICE_ATTR(file_cache_rd, S_IRUGO  | S_IWUSR, file_cache_rd_show, file_cache_rd_store);
static DEVICE_ATTR(cache_file_path, S_IRUGO, cache_file_path_show, NULL);
static DEVICE_ATTR(mask_file_path, S_IRUGO, mask_file_path_show, NULL);
			...
			const struct i2c_device_id *id)
{
...
#ifdef EEPROM_CLASS
		strlcpy(port_name, chip.eeprom_data->label, MAX_PORT_NAME_LEN);
#else
    ...
    snprintf(optoe->cache_file_path, MAX_NAME_SIZE, OPTOE_CACHE_FILE_PATH, client->adapter->nr, client->addr);
    snprintf(optoe->mask_file_path, MAX_NAME_SIZE, OPTOE_MASK_FILE_PATH, client->adapter->nr, client->addr);
