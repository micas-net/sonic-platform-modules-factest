...
static ssize_t slot_card_type_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int slot_index;

    check_p(g_slot_drv);
    check_p(g_slot_drv->get_slot_card_type);

    slot_index = obj->index;
    SLOT_DBG("slot index: %u\n", slot_index);
    return g_slot_drv->get_slot_card_type(slot_index, buf, PAGE_SIZE);
}

static ssize_t slot_present_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int slot_index;

    check_p(g_slot_drv);
    check_p(g_slot_drv->get_slot_present);

    slot_index = obj->index;
    SLOT_DBG("slot index: %u\n", slot_index);
    return g_slot_drv->get_slot_present(slot_index, buf, PAGE_SIZE);
}
...
static ssize_t slot_power_ctrl_status_show(struct switch_obj *obj, struct switch_attribute *attr,
                   char *buf)
{
    unsigned int slot_index;

    check_p(g_slot_drv);
    check_p(g_slot_drv->get_slot_power_ctrl_status);

    slot_index = obj->index;
    SLOT_DBG("slot index: %u\n", slot_index);
    return g_slot_drv->get_slot_power_ctrl_status(slot_index, buf, PAGE_SIZE);
}

static ssize_t slot_power_ctrl_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char *buf, size_t count)
{
    unsigned int slot_index;
    int ret, power;
    ...
    ret = g_slot_drv->set_slot_power_ctrl_status(slot_index, power);
    if (ret < 0) {
...
static struct switch_attribute slot_card_type_attr = __ATTR(slot_card_type, S_IRUGO, slot_card_type_show, NULL);
static struct switch_attribute slot_present_attr = __ATTR(present, S_IRUGO, slot_present_show, NULL);
...
static struct switch_attribute slot_power_on_attr = __ATTR(power_on, S_IRUGO | S_IWUSR, slot_power_ctrl_status_show, slot_power_ctrl_store);
...
static struct attribute *slot_attrs[] = {
    ...
    &slot_sn_attr.attr,
    &slot_pn_attr.attr,
    &slot_card_type_attr.attr,
    &slot_present_attr.attr,
