                   ...
                   const char* buf, size_t count)
{
    ...
    if (ret < 0) {
        VOL_SENSOR_ERR("set vol%u max threshold failed, value: %s, count: %zu, ret: %d\n",
            vol_index, buf, count, ret);
        return ret;
    }
    VOL_SENSOR_DBG("set vol%u max threshold success, value: %s, count: %zu, ret: %d\n",
        vol_index, buf, count, ret);
    return count;
}
                   ...
                   const char* buf, size_t count)
{
    ...
    if (ret < 0) {
        VOL_SENSOR_ERR("set vol%u min threshold failed, value: %s, count: %zu, ret: %d\n",
            vol_index, buf, count, ret);
        return ret;
    }
    VOL_SENSOR_DBG("set vol%u min threshold success, value: %s, count: %zu, ret: %d\n",
        vol_index, buf, count, ret);
    return count;
}
