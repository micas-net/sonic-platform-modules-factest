                   ...
                   const char* buf, size_t count)
{
    ...
    if (ret < 0) {
        PSU_ERR("set psu%u temp%u max threshold failed, value: %s, count: %zu, ret: %d\n",
            psu_index, temp_index, buf, count, ret);
        return ret;
    }
    PSU_DBG("set psu%u temp%u max threshold success, value: %s, count: %zu, ret: %d\n",
        psu_index, temp_index, buf, count, ret);
    return count;
}
                   ...
                   const char* buf, size_t count)
{
    ...
    if (ret < 0) {
        PSU_ERR("set psu%u temp%u min threshold failed, value: %s, count: %zu, ret: %d\n",
            psu_index, temp_index, buf, count, ret);
        return ret;
    }
    PSU_DBG("set psu%u temp%u min threshold success, value: %s, count: %zu, ret: %d\n",
        psu_index, temp_index, buf, count, ret);
    return count;
}
