                   ...
                   const char* buf, size_t count)
{
    ...
    if (ret < 0) {
        CURR_SENSOR_ERR("set curr%u max threshold failed, value: %s, count: %zu, ret: %d\n",
            curr_index, buf, count, ret);
        return ret;
    }
    CURR_SENSOR_DBG("set curr%u max threshold success, value: %s, count: %zu, ret: %d\n",
        curr_index, buf, count, ret);
    return count;
}
                   ...
                   const char* buf, size_t count)
{
    ...
    if (ret < 0) {
        CURR_SENSOR_ERR("set curr%u min threshold failed, value: %s, count: %zu, ret: %d\n",
            curr_index, buf, count, ret);
        return ret;
    }
    CURR_SENSOR_DBG("set curr%u min threshold success, value: %s, count: %zu, ret: %d\n",
        curr_index, buf, count, ret);
    return count;
}
