...
static ssize_t transceiver_power_ctrl_bitmap_set(const char* buf, size_t count)
{
    ...
    SFF_DBG("Start, power ctrl count: %lu. buf: %s\n", count, buf);
    ...
    for (i = 0; i < actual_strlen; i++) {
        ...
        if (value != 0 && value != 1) {
            SFF_ERR("invaild parameter (must be '0' or '1'), eth%d count: %lu. buf: %s.\n", i + 1, count, buf);
            return -EINVAL;
        }
    ...
    if (update_group_res != 1) {
        SFF_DBG("Not all group update OK. count: %lu, buf: %s.\n", count, buf);
        return -EBADRQC;
    }

    SFF_DBG("All groups update OK. count: %lu, buf: %s.\n", count, buf);
    return count;
}
                                    ...
                                    const char* buf, size_t count)
{
    ...
    /* Validate the character (must be '0' or '1') */
    if (value != 0 && value != 1) {
        SFF_ERR("invalid parameter (must be '0' or '1'), eth%d count: %lu, buf: %s.\n", eth_index, count, buf);
        return -EINVAL;
    }
