...
/**
 * dfd_ko_cpld_logic_dev_read - cpld read operation
 * @offset: Offset address
 * @buf: data
 *
 * @returns: <0 Failure, other success
 */
static int32_t dfd_ko_cpld_logic_dev_read(int32_t addr, uint8_t *buf)
{
    uint64_t key;
    int sub_slot, cpld_id, offset;
    char *logic_dev_path;
    int rd_len;
    ...
    /* Obtain the logic_dev device addr */
    sub_slot = DFD_KO_CPLD_GET_SLOT(addr);
    cpld_id = DFD_KO_CPLD_GET_ID(addr);
...
/**
 * dfd_ko_cpld_logic_dev_write - cpld WRITE OPERATION
 * @offset: Offset address
 * @buf: data
 *
 * @returns: <0 Failure, other success
 */
static int32_t dfd_ko_cpld_logic_dev_write(int32_t addr, uint8_t data)
{
    int rv;
    uint64_t key;
    int sub_slot, cpld_id, offset;
...
static int dfd_cfg_get_cpld_mode(int sub_slot, int cpld_id, int *mode)
{
    ...
    } else if (!strncmp(name, DFD_KO_CPLD_MODE_LOGIC_DEV_STRING, strlen(DFD_KO_CPLD_MODE_LOGIC_DEV_STRING))) {
        *mode = DFD_CPLD_MODE_LOGIC_DEV;
    } else {
...
#endif
    } else if (cpld_mode == DFD_CPLD_MODE_LOGIC_DEV) {
        ret = dfd_ko_cpld_logic_dev_read(addr, buf);
    } else {
...
int32_t dfd_ko_cpld_write(int32_t addr, uint8_t val)
{
    ...
    if (cpld_mode == DFD_CPLD_MODE_I2C) {
        ...
        ret = dfd_ko_cpld_logic_dev_write(addr, val);
    } else {
        DBG_DEBUG(DBG_ERROR, "cpld_mode %d invalid.\n", cpld_mode);
...
int32_t dfd_ko_other_i2c_dev_read(int32_t addr, uint8_t *value, int32_t read_len)
{
    ...
    if (rv < 0) {
        DBG_DEBUG(DBG_ERROR, "dfd_ko_other_i2c_dev_read failed, offset: 0x%x, read_len: %d, rv: %d\n",
            addr, read_len, rv);
    } else {
        DBG_DEBUG(DBG_VERBOSE, "dfd_ko_other_i2c_dev_read success, offset: 0x%x, read_len: %d\n",
            addr, read_len);
        for (i = 0; i < read_len; i++) {
            DBG_DEBUG(DBG_VERBOSE, "dfd_ko_other_i2c_dev_read, value[%d] = 0x%x\n", i, value[i]);
        }
    }
...
int32_t dfd_ko_write_file(char *fpath, int32_t addr, uint8_t *val, int32_t write_bytes)
{
    ...
    if ((fpath == NULL) || (val == NULL) || (addr < 0) || (write_bytes <= 0)) {
     ...
     /* Open file */
    filp = filp_open(fpath, O_WRONLY, 777);
    if (IS_ERR(filp)) {
        DBG_DEBUG(DBG_ERROR, "open file[%s] fail\n", fpath);
        return -DFD_RV_DEV_FAIL;
    }
