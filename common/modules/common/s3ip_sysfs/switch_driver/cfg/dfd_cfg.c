...
/* Power name Convert psu type head */
LIST_HEAD(dfd_lib_cfg_power_name_conv_lst);

/* Slot name Convert to slot card type head */
LIST_HEAD(dfd_lib_cfg_slot_card_type_to_name_conv_lst);
                ...
                int index1, int index2)
{
    ...
    val_convert_node_t *tmp_val_convert;

    list_for_each_entry(tmp_val_convert, root, lst) {
        if ((tmp_val_convert->index1 == index1) && (tmp_val_convert->index2 == index2)) {
            if (str != NULL) {
                strlcpy(tmp_val_convert->str_val, str, sizeof(tmp_val_convert->str_val));
            }
            tmp_val_convert->int_val = val;
            return;
        }
    }
                ...
                int index1, int index2)
{
    ...
    } else if (cfg_item_id == DFD_CFG_ITEM_SLOT_CARD_TYPE_TO_NAME) {
        dfd_ko_cfg_regval_conv_lst_add(&dfd_lib_cfg_slot_card_type_to_name_conv_lst, val, str, index1, index2);
    }
    return;
}
...
/**
 * dfd_ko_cfg_get_slot_card_type_by_name - obtain the slot card_type by slot name
 * @slot_name: slot name
...
int dfd_ko_cfg_get_slot_card_type_by_name(char *slot_name, int *slot_card_type)
{
    ...
    if ((slot_name == NULL) || (slot_card_type == NULL)) {
        DBG_DEBUG(DBG_ERROR, "input arguments error\n");
        return -1;
    }

    rv = dfd_ko_cfg_get_index_by_strval(&dfd_lib_cfg_slot_card_type_to_name_conv_lst, slot_name, &index1, &index2);
    if (rv < 0) {
        DBG_DEBUG(DBG_ERROR, "get slot card type by name[%s] fail\n", slot_name);
        return -1;
    }

    *slot_card_type = index1;
                ...
                char *buf_val, int line_num)
{
    ...
    case INFO_CTRL_MEM_POLA:
        ...
        break;
    case INFO_CTRL_MEM_VAL_POLA:
        info_ctrl->val_pola = dfd_ko_cfg_get_enum_value_by_str(g_info_pola_str, INFO_POLA_END, buf_val);
        break;
    ...
    case INFO_CTRL_MEM_LEN:
        ...
        break;
    case INFO_CTRL_MEM_VAL_LEN:
        dfd_ko_cfg_get_value_from_char(buf_val, &(info_ctrl->val_len), line_num);
        break;
...
static void dfd_ko_cfg_print_item(uint64_t key, const void *cfg)
{
    ...
    if (DFD_CFG_ITEM_IS_INT(item_id)) {
        ...
        printk(KERN_INFO ".val_pola=%s\n", g_info_pola_str[info_ctrl->val_pola]);
        printk(KERN_INFO ".pola=%s\n", g_info_pola_str[info_ctrl->pola]);
        printk(KERN_INFO ".fpath=%s\n", info_ctrl->fpath);
        printk(KERN_INFO ".addr=0x%02x\n", info_ctrl->addr);
        printk(KERN_INFO ".val_len=%d\n", info_ctrl->val_len);
...
static int dfd_ko_cfg_init(bool re_init)
{
    int rv;
    int card_type;
    char file_name[32] = {0};
...
{
    return dfd_ko_cfg_init(re_init);
}
...
void dfd_dev_cfg_exit(void)
{
    ...
    val_convert_node_lst_free(&dfd_lib_cfg_slot_card_type_to_name_conv_lst);
    return;
}
