...
static int pca9641_gpio_init(gpio_attr_t *gpio_attr)
{
    ...
    return 0;
error:
    DEBUG_ERROR("pca9641_gpio_init failed, ret:%d.\n", err);
    return err;
}
...
static int pca9641_do_file_reset(struct i2c_mux_core *muxc)
{
    ...
    DEBUG_VERBOSE("dev_name:%s, offset:0x%x, mask:0x%x, on:0x%x, off:0x%x.\n",
    ...
    while (timeout > 0) {
        ...
        if (val == file_attr->reset_off) {
            ret = 0;
            DEBUG_VERBOSE("pca9641_do_file_reset success.\n");
            break;
        }
        udelay_cnt++;
    ...
    if (ret < 0) {
        DEBUG_ERROR("pca9641_do_file_reset timeout.\n");
    }
out:
    if (err < 0) {
        DEBUG_ERROR("pca9641_do_file_reset file rd/wr failed, ret:%d.\n", err);
    }
...
static int pca9641_do_io_reset(struct i2c_mux_core *muxc)
{
    ...
    while (timeout > 0) {
        ...
        if (val == io_attr->reset_off) {
            ret = 0;
            DEBUG_VERBOSE("pca9641_do_io_reset success.\n");
            break;
        }
        udelay_cnt++;
        if ((udelay_cnt % 1000) == 0) {
    ...
    if (ret < 0) {
        DEBUG_ERROR("pca9641_do_io_reset timeout.\n");
    }
...
static int pca9641_do_gpio_reset(struct i2c_mux_core *muxc)
{
    ...
    while (timeout > 0) {
        ...
        if (val == gpio_attr->reset_off) {
            ret = 0;
            DEBUG_VERBOSE("pca9641_do_gpio_reset success.\n");
            break;
        }
        udelay_cnt++;
        if ((udelay_cnt % 1000) == 0) {
    ...
    if (ret < 0) {
        DEBUG_ERROR("pca9641_do_gpio_reset timeout.\n");
    }
...
static int pca9641_do_i2c_reset(struct i2c_mux_core *muxc)
{
    ...
    while (timeout > 0) {
        ...
        if (val == i2c_attr->reset_off) {
            ret = 0;
            DEBUG_VERBOSE("pca9641_do_i2c_reset success.\n");
            break;
        }
        udelay_cnt++;
        if ((udelay_cnt % 1000) == 0) {
    ...
    if (ret < 0) {
        DEBUG_ERROR("pca9641_do_i2c_reset timeout.\n");
    }
out:
    if (err < 0) {
        DEBUG_ERROR("pca9641_do_i2c_reset i2c op failed, ret:%d.\n", err);
    }
    return ret;
}
...
static int pca9641_do_reset(struct i2c_mux_core *muxc)
{
    ...
    if (ret < 0) {
        DEBUG_ERROR("pca9641_reset_ctrl failed, reset type:%u, ret:%d.\n",
...
static int pca9641_arbitrate(struct i2c_client *client)
{
    ...
    if (reg_ctl < 0) {
        DEBUG_ERROR("pca9641 read control register failed, ret:%d.\n", reg_ctl);
        return reg_ctl;
    }

    reg_sts = pca9541_reg_read(client, PCA9641_STATUS);
    if (reg_sts < 0) {
        DEBUG_ERROR("pca9641 read status register failed, ret:%d.\n", reg_sts);
        return reg_sts;
    }
    ...
    if (BUSOFF(reg_ctl, reg_sts)) {
        ...
        reg_ctl = pca9541_reg_read(client, PCA9641_CONTROL);
        if (reg_ctl < 0) {
            DEBUG_ERROR("Bus is off, but read control register failed, ret:%d.\n", reg_ctl);
            return reg_ctl;
        }
        ...
        if (lock_grant(reg_ctl)) {
            ...
            * or arbitration timeout expired. Take the bus.
            */
            DEBUG_VERBOSE("Bus is off, get pca9641 arbitration success.\n");
            ...
            * Set extra long timeout to give it time to acquire it.
            */
            DEBUG_VERBOSE("Bus is off, but get pca9641 arbitration failed.\n");
            data->select_timeout = SELECT_DELAY_LONG * 2;
        }
        ...
        * Bus is on, and we own it. We are done with acquisition.
        */
        DEBUG_VERBOSE("Bus is on, get pca9641 arbitration success.\n");
        reg_ctl |= PCA9641_CTL_BUS_CONNECT | PCA9641_CTL_LOCK_REQ;
        pca9541_reg_write(client, PCA9641_CONTROL, reg_ctl);
        return 1;
        ...
        * Otherwise request it.
        */
        DEBUG_VERBOSE("Other master owns the bus, try to request it.\n");
        data->select_timeout = SELECT_DELAY_LONG;
        reg_ctl |= PCA9641_CTL_LOCK_REQ;
        pca9541_reg_write(client, PCA9641_CONTROL, reg_ctl);
    }
...
static int pca9641_select_chan(struct i2c_mux_core *muxc, u32 chan)
{
    ...
    if (ret < 0) {
        DEBUG_ERROR("pca9641 select channel failed, ret:%d, try to reset pca9641.\n", ret);
        rv = pca9641_do_reset(muxc);

        if (rv < 0) {
            DEBUG_ERROR("pca9641 reset failed, rv:%d.\n", rv);
            return ret;
        }

        ret = pca9641_select_chan_single(muxc, chan);
        if (ret < 0) {
            DEBUG_ERROR("after pca9641 reset, select channel still failed, ret:%d.\n", ret);
        }
    }
    return ret;
}
...
static int of_pca9641_reset_data_init(struct pca9541 *data)
{
    ...
    if (of_property_read_u32(dev->of_node, "pca9641_reset_type", &reset_cfg->pca9641_reset_type)) {
        ...
        DEBUG_VERBOSE("pca9641_reset_type not found, no reset.\n");
        reset_cfg->pca9641_reset_type = PCA9641_RESET_NONE;
        return 0;
    }
    ...
    if (reset_cfg->pca9641_reset_type == PCA9641_RESET_I2C) {
        ...
        DEBUG_VERBOSE("dev_name:%s, mask:0x%x, reset_on:0x%x, reset_off:0x%x.\n",
...
static int pca9641_reset_data_init(struct pca9541 *data)
{
    ...
    if (data->client->dev.platform_data == NULL) {
        DEBUG_VERBOSE("pca9641 has no reset platform data config.\n");
        return 0;
    }
    reset_cfg = &data->pca9641_cfg_info;
    ...
    if (reset_cfg->pca9641_reset_type == PCA9641_RESET_NONE) {
        DEBUG_VERBOSE("pca9641 has no reset function.\n");
        return 0;
    }
...
static int pca9541_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (detect_id == 0) { /* pca9541 */
        ...
        if (of_property_read_u32(client->dev.of_node, "pca9641_nr", &data->pca9641_nr)) {
            ...
            force = 0;
            DEBUG_VERBOSE("pca9641_nr not found, use dynamic adap number.\n");
        } else {
            force = data->pca9641_nr;
            DEBUG_VERBOSE("pca9641_nr: %d.\n", force);
        }
        ...
        if (client->dev.of_node == NULL) {
            if (client->dev.platform_data == NULL) {
                ...
                if (data->pca9641_nr == 0) {
                    force = 0;
                    DEBUG_VERBOSE("pca9641_nr = 0, use dynamic adap number.\n");
                } else {
                    force = data->pca9641_nr;
                    DEBUG_VERBOSE("pca9641_nr: %d.\n", force);
                }
