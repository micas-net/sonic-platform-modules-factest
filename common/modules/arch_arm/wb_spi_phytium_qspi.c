	...
	if (!qspi->nodirmap) {
		...
		for (i = 1; qspi->fnum > i && i < PHYTIUM_QSPI_MAX_NORCHIP; i++) {
			if (qspi->flash[i].size != qspi->flash[0].size) {
				dev_warn(dev, "Flash %d size is invalid. size: %d\n", i, qspi->flash[i].size);
#if 0
