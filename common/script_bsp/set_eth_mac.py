...
def increment_mac_address(base_mac, increment):
    """Calculate the final MAC address based on the base MAC address and the increment."""
    # Convert the base MAC address to an integer.
    mac_int = int(base_mac.replace(':', ''), 16)
    # Increase the increment.
    final_mac_int = mac_int + increment
    # Convert the final MAC address back to string format.
    final_mac = ':'.join(f'{(final_mac_int >> (i * 8)) & 0xff:02x}' for i in reversed(range(6)))
    return final_mac
...
def check_eth_status(eth_name):
    """check net interface status"""
    try:
        path = "/sys/class/net/%s/operstate" % (eth_name)
...
def set_eth_mac(eth_name, mac):
    try:
      eth_status = check_eth_status(eth_name)
      set_mac_cmd = "ifconfig %s hw ether %s" % (eth_name, mac)
      if eth_status is None or eth_status == "up":
          #set mac when eth is down
          set_mac_up_cmd = "ifconfig %s up" % (eth_name)
          set_mac_down_cmd = "ifconfig %s down" % (eth_name)
          status, output = exec_os_cmd(set_mac_down_cmd)
          if status:
              setmac_error("run cmd: %s fail, msg: %s" % (set_mac_down_cmd, output))
              return False
          status, output = exec_os_cmd(set_mac_cmd)
          if status:
              setmac_error("run cmd: %s fail, msg: %s" % (set_mac_cmd, output))
              return False
          status, output = exec_os_cmd(set_mac_up_cmd)
          if status:
              setmac_error("run cmd: %s fail, msg: %s" % (set_mac_up_cmd, output))
              return False
      else:
          #set mac directly
          status, output = exec_os_cmd(set_mac_cmd)
...
def doSetmac():
    try:
        ...
        for setmac_item in SET_MAC_CONF:
            ...
            increment = setmac_item.get("increment", 0)
            ...
            status, base_e2_mac = get_mac_from_eeprom(setmac_item)
            if status is False:
                setmac_error("get mac from %s eeprom fail, msg: %s" % (e2_name, base_e2_mac))
                continue
            status = validate_mac(base_e2_mac)
            if status is False:
                setmac_error("base_e2_mac: %s invalid" % base_e2_mac)
                continue
            setmac_debug("get mac from %s eeprom info success, mac: %s" % (e2_name, base_e2_mac))
            e2_mac = increment_mac_address(base_e2_mac, increment)
            setmac_debug("e2_mac: %s" % (e2_mac))
