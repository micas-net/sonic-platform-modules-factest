...
SUB_VERSION_FILE = "/etc/device/.subversion"
...
def removeDev(bus, loc):
    log_message("%%PLATFORM_BASE: removeDev, bus: %s, loc: %s" % (bus, loc))
...
def addDev(name, bus, loc):
    log_message("%%PLATFORM_BASE: addDev, name: %s, bus: %s, loc: %s" % (name, bus, loc))
...
def adddriver(name, delay):
    log_message("%%PLATFORM_BASE: adddriver, driver name: %s, delay: %s" % (name, delay))
    cmd = "modprobe %s" % name
    if delay != 0:
        time.sleep(delay)
...
def removedriver(name, delay, removeable=1):
    ...
    log_message("%%PLATFORM_BASE: removedriver, driver name: %s, delay: %s, removeable: %s" %
        (realname, delay, removeable))
