...
import click
import traceback
from platform_util import get_value, set_value, exec_os_cmd, exec_os_cmd_log, write_sysfs, check_value
...
class BasePlatform():
    ...
    def do_fw_upg_cmd_once(self, dev_name, cmd_config):
        ...
        if operation != "set_value" and operation != "check_value":
    ...
    def card_id_check(self, e2_card_id_config, header_card_id, card_flag):
        try:
            ...
            if len(e2_card_id_config) != 0:
                ...
                e2_id = int(get_board_id(), base = 16)
    ...
    def do_upgrade_test(self, file, slot, upg_type, success_chain, fail_chain):
        try:
            file_test_list = []
            ret, log = self.initial_check(file, slot, upg_type, None, file_test_list, None, None)
            if ret != CHECK_OK:
                return ret, log
    ...
    def do_test(self, device, slot):
        try:
            ...
            for test_config in device_list:
                ...
                ret, log = self.do_upgrade_test(test_file, slot, TEST_UPGRADE, success_chain, fail_chain)
